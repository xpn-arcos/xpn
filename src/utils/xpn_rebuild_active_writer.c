...
int copy(char *entry, int is_file, int blocksize, int replication_level, int rank, int size) {
    ...
    // Calculate the master node of the new file
    for (int i = 0; i < size; i++) {
        if (rank_actual_to_new[i] == 0) {
            master_new = i;
        }
    }
    ...
    if (!is_file) {
        ...
        do {
            ...
            if (rank_actual_to_old[rank] != -1) {
                MPI_Status status;
                MPI_Recv(&offset_src, 1, MPI_LONG, MPI_ANY_SOURCE, TAG_OFFSET, MPI_COMM_WORLD, &status);
                ...
                MPI_Send(&read_size, 1, MPI_LONG, status.MPI_SOURCE, TAG_READ_SIZE, MPI_COMM_WORLD);

                MPI_Send(buf, read_size, MPI_CHAR, status.MPI_SOURCE, TAG_BUF, MPI_COMM_WORLD);
            }
            ...
            if (rank_actual_to_new[rank] != -1) {
                ...
                MPI_Recv(&read_size, 1, MPI_LONG, rank_old_to_actual[rank_to_send], TAG_READ_SIZE, MPI_COMM_WORLD,
                         MPI_STATUS_IGNORE);
                MPI_Recv(buf, read_size, MPI_CHAR, rank_old_to_actual[rank_to_send], TAG_BUF, MPI_COMM_WORLD,
                         MPI_STATUS_IGNORE);
                if (read_size <= 0) {
                    break;
                }
        ...
        if (rank == master_new) {
            for (int i = 0; i < size; i++) {
                if (rank_actual_to_old[i] != -1) {
                    offset_src = -666;
                    MPI_Send(&offset_src, 1, MPI_LONG, i, TAG_OFFSET, MPI_COMM_WORLD);
                }
            }
        }
