...
//	char *st;
	...
	xpn_fgets( buf, 17, f );		/* skip header bytes */
	...
	xpn_fgets( buf, 21, f );	/* skip rest of header */
	...
	for (row=0; row < image->rows; row++)
	{
	   ...
	   {
		buf[ 2 ] = xpn_fgetc( f );	/* blue */
		buf[ 1 ] = xpn_fgetc( f );	/* green */
		buf[ 0 ] = xpn_fgetc( f );  /* red */
	   ...
	   if (padding > 0) xpn_fgets( buf, padding+1, f ); 
	}
...
//	unsigned int r, g, b;
	...
	buf[0] = 'B';
	buf[1] = 'M';
	xpn_fwrite( buf, 1, 2, f );
	
	padding = ((int)((double)image->cols / 4.0 + 0.5))*4 - image->cols;
	longtmp = image->cols*image->rows*3 + padding*image->rows + 54;
	xpn_fwrite( &longtmp, sizeof(longtmp), 1, f );
	...
	xpn_fwrite( &longtmp, sizeof(longtmp), 1, f );
	shortmp = 1;
	xpn_fwrite( &shortmp, sizeof(shortmp), 1, f );
	shortmp = 24;
	xpn_fwrite( &shortmp, sizeof(shortmp), 1, f );
	longtmp = 0;
	xpn_fwrite( &longtmp, sizeof(longtmp), 1, f );
	...
	for (row=0; row < image->rows; row++)
	{
	   for (col=0; col < image->cols; col++)
	   {
		...
		xpn_fwrite( buf, 1, 3, f );
	   }
	   
	   /* padding */
	   padding = ((int)((double)image->cols / 4.0 + 0.5))*4 - image->cols;
	   if (padding > 0) xpn_fwrite( buf, 1, padding, f ); 
	}
