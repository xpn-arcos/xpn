...
void *write_func ( void *func_args )
{
	...
	while (sum < st.st_size) {
		...
		if (args->xpndest) {
			...
			nw = xpn_fwrite(array_bufs[next]->buffer, 1, array_bufs[next]->buffer_length, args->fdd);
			if (args->xpnsource)
				pthread_mutex_unlock(&xpn_mutex);
		} else {
			nw = fwrite(array_bufs[next]->buffer, 1, array_bufs[next]->buffer_length, args->fdd);
			fflush(args->fdd); // Improves performance
		}
	...
	do {
		...
		if (xpnsource) {
			...
			if (xpndest)
				pthread_mutex_lock(&xpn_mutex);
			if (do_prefetch)
				nr = xpn_fread_prefetch(buf->buffer, 1, buffer_size, fds);
			else
				nr = xpn_fread(buf->buffer, 1, buffer_size, fds);
			if (xpndest)
				pthread_mutex_unlock(&xpn_mutex);
		} else
			nr = fread(buf->buffer, 1, buffer_size, fds);
		if (!very_silent) {
			gettimeofday(&t_end_trans, NULL);
