...
void *write_func(void *func_args)
{
	...
	while (sum < st.st_size)
	{
		...
		if (debug)
			printf("Written a block of %zd bytes\n", sum_w);
	}
...
int main(int argc, char *argv[])
{
	...
	do {
		...
		if (debug) {
			...
			if (xpnsource && xpndest)
				fprintf(stderr, " read(%zu)    read block %zu (%p) of %zd bytes\n", buffer_size, last, buf->buffer, nr);
			else
				fprintf(stderr, " read(%zu)    read block of %zd bytes\n", buffer_size, nr);
		}
