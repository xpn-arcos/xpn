...
/* ... Functions / Funciones ......................................... */
    ...
    void xpn_server_op_creat(xpn_server_param_st * params, void * comm, struct st_xpn_server_msg * head, int rank_client_id, int tag_client_id)
    {
        ...
        path_len = head -> u_st_xpn_server_msg.op_creat.path_len;
        memcpy(full_path, head -> u_st_xpn_server_msg.op_creat.path, path_len > XPN_PATH_MAX ? XPN_PATH_MAX : path_len);
        if (path_len > XPN_PATH_MAX) {
            ssize_t r = socket_recv(* (int *)comm, full_path + XPN_PATH_MAX, path_len - XPN_PATH_MAX);
            if (r < 0) {
                full_path[0] = '\0';
    ...
    void xpn_server_op_read(xpn_server_param_st * params, void * comm, struct st_xpn_server_msg * head, int rank_client_id, int tag_client_id)
    {
        ...
        path_len = head -> u_st_xpn_server_msg.op_read.path_len;
        memcpy(full_path, head -> u_st_xpn_server_msg.op_read.path, path_len > XPN_PATH_MAX ? XPN_PATH_MAX : path_len);
        if (path_len > XPN_PATH_MAX) {
            ssize_t r = socket_recv(* (int *)comm, full_path + XPN_PATH_MAX, path_len - XPN_PATH_MAX);
            if (r < 0) {
                full_path[0] = '\0';
...
cleanup_xpn_server_op_write:
    ...
    void xpn_server_op_close(xpn_server_param_st * params, void * comm, struct st_xpn_server_msg * head, int rank_client_id, int tag_client_id)
    {
        ...
        path_len = head -> u_st_xpn_server_msg.op_close.path_len;
        memcpy(full_path, head -> u_st_xpn_server_msg.op_close.path, path_len > XPN_PATH_MAX ? XPN_PATH_MAX : path_len);
        if (path_len > XPN_PATH_MAX) {
            ssize_t r = socket_recv(* (int *)comm, full_path + XPN_PATH_MAX, path_len - XPN_PATH_MAX);
            if (r < 0) {
                full_path[0] = '\0';
    ...
    void xpn_server_op_rm ( xpn_server_param_st * params, void * comm, struct st_xpn_server_msg * head, int rank_client_id, int tag_client_id )
    {
        struct st_xpn_server_status status;
        int path_len = 0;
        char full_path[2048]; // TODO: 2048 ->  PATH_MAX (!)
        ...
        path_len = head -> u_st_xpn_server_msg.op_rm.path_len;
        memcpy(full_path, head -> u_st_xpn_server_msg.op_rm.path, path_len > XPN_PATH_MAX ? XPN_PATH_MAX : path_len);
        if (path_len > XPN_PATH_MAX) {
            ssize_t r = socket_recv(* (int *)comm, full_path + XPN_PATH_MAX, path_len - XPN_PATH_MAX);
            if (r < 0) {
                full_path[0] = '\0';
    ...
    {
        struct st_xpn_server_status status;
        int old_path_len = 0, new_path_len = 0;
        char full_path_old[2048], full_path_new[2048];
        bzero(full_path_old, 2048);
        bzero(full_path_new, 2048);
        ...
        old_path_len = head -> u_st_xpn_server_msg.op_rename.old_url_len;
        memcpy(full_path_old, head -> u_st_xpn_server_msg.op_rename.old_url, old_path_len > XPN_PATH_MAX ? XPN_PATH_MAX : old_path_len);
        if (old_path_len > XPN_PATH_MAX) {
            ssize_t r = socket_recv(* (int *)comm, full_path_old + XPN_PATH_MAX, old_path_len - XPN_PATH_MAX);
            if (r < 0) {
                full_path_old[0] = '\0';
            }
        }
        full_path_old[old_path_len] = '\0';

        new_path_len = head -> u_st_xpn_server_msg.op_rename.new_url_len;
        memcpy(full_path_new, head -> u_st_xpn_server_msg.op_rename.old_url, new_path_len > XPN_PATH_MAX ? XPN_PATH_MAX : new_path_len);
        if (new_path_len > XPN_PATH_MAX) {
            ssize_t r = socket_recv(* (int *)comm, full_path_new + XPN_PATH_MAX, new_path_len - XPN_PATH_MAX);
            if (r < 0) {
                full_path_new[0] = '\0';
            }
        }
        full_path_new[new_path_len] = '\0';
    ...
    void xpn_server_op_getattr(xpn_server_param_st * params, void * comm, struct st_xpn_server_msg * head, int rank_client_id, int tag_client_id)
    {
        ...
        int path_len = 0;
        char full_path[2048]; // TODO: PATH_MAX
        ...
        path_len = head -> u_st_xpn_server_msg.op_getattr.path_len;
        memcpy(full_path, head -> u_st_xpn_server_msg.op_getattr.path, path_len > XPN_PATH_MAX ? XPN_PATH_MAX : path_len);
        if (path_len > XPN_PATH_MAX) {
            ssize_t r = socket_recv(* (int *)comm, full_path + XPN_PATH_MAX, path_len - XPN_PATH_MAX);
            if (r < 0) {
                full_path[0] = '\0';
    ...
    void xpn_server_op_mkdir(xpn_server_param_st * params, void * comm, struct st_xpn_server_msg * head, int rank_client_id, int tag_client_id)
    {
        ...
        path_len = head -> u_st_xpn_server_msg.op_mkdir.path_len;
        memcpy(full_path, head -> u_st_xpn_server_msg.op_mkdir.path, path_len > XPN_PATH_MAX ? XPN_PATH_MAX : path_len);
        if (path_len > XPN_PATH_MAX) {
            ssize_t r = socket_recv(* (int *)comm, full_path + XPN_PATH_MAX, path_len - XPN_PATH_MAX);
            if (r < 0) {
                full_path[0] = '\0';
    ...
    void xpn_server_op_opendir(xpn_server_param_st * params, void * comm, struct st_xpn_server_msg * head, int rank_client_id, int tag_client_id)
    {
        ...
        path_len = head -> u_st_xpn_server_msg.op_opendir.path_len;
        memcpy(full_path, head -> u_st_xpn_server_msg.op_opendir.path, path_len > XPN_PATH_MAX ? XPN_PATH_MAX : path_len);
        if (path_len > XPN_PATH_MAX) {
            ssize_t r = socket_recv(* (int *)comm, full_path + XPN_PATH_MAX, path_len - XPN_PATH_MAX);
            if (r < 0) {
                full_path[0] = '\0';
    ...
    void xpn_server_op_readdir(xpn_server_param_st * params, void * comm, struct st_xpn_server_msg * head, int rank_client_id, int tag_client_id)
    {
        ...
        path_len = head -> u_st_xpn_server_msg.op_readdir.path_len;
        memcpy(full_path, head -> u_st_xpn_server_msg.op_readdir.path, path_len > XPN_PATH_MAX ? XPN_PATH_MAX : path_len);
        if (path_len > XPN_PATH_MAX) {
            ssize_t r = socket_recv(* (int *)comm, full_path + XPN_PATH_MAX, path_len - XPN_PATH_MAX);
            if (r < 0) {
                full_path[0] = '\0';
    ...
    void xpn_server_op_rmdir(xpn_server_param_st * params, void * comm, struct st_xpn_server_msg * head, int rank_client_id, int tag_client_id)
    {
        ...
        path_len = head -> u_st_xpn_server_msg.op_rmdir.path_len;
        memcpy(full_path, head -> u_st_xpn_server_msg.op_rmdir.path, path_len > XPN_PATH_MAX ? XPN_PATH_MAX : path_len);
        if (path_len > XPN_PATH_MAX) {
            ssize_t r = socket_recv(* (int *)comm, full_path + XPN_PATH_MAX, path_len - XPN_PATH_MAX);
            if (r < 0) {
                full_path[0] = '\0';
...
cleanup_xpn_server_op_write_mdata:
    ...
    pthread_mutex_t op_write_mdata_file_size_mutex = PTHREAD_MUTEX_INITIALIZER; // TODO: why this global variable ??????
