 ...
 * @return 'TODO'.
 */
const char *skip_xpn_prefix(const char *path) { return (const char *)(path + strlen(xpn_adaptor_partition_prefix)); }
...
void fdstable_realloc(void) {
    ...
    if (NULL == fdstable) {
        ...
        fdstable =
            (struct generic_fd *)realloc((struct generic_fd *)fdstable, fdstable_size * sizeof(struct generic_fd));
    }
 ...
 *    Nothing.
 */
void fdstable_init(void) { fdstable_realloc(); }
 ...
 *    Nothing.
 */
void fdsdirtable_init(void) { fdsdirtable_realloc(); }
...
struct dirent64 *readdir64(DIR *dirp) {
    ...
    if (fdsdirtable_get(dirp) != -1) {
        ...
        if (aux != NULL) {
            ret = (struct dirent64 *)malloc(
                sizeof(struct dirent64));  // TODO: change to static memory per dir... or where memory is free?
