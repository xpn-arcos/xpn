  ...
  /* ... Auxiliar Functions / Funciones Auxiliares ..................... */
    ...
    void tcp_server_dispatcher ( struct st_th th )
    {
      ...
      while (!disconnect)
      {
        ...
        if (ret == -1) {
          debug_info("[MPI-SERVER] ERROR: tcp_server_comm_readdata fail\n") ;
          return;
        }
      ...
      debug_info("[MPI-SERVER] tcp_server_worker_run (ID=%d) close\n", th.rank_client_id);

      tcpClient_comm_close((int)th.sd) ;
    }
  ...
  /* ... Functions / Funciones ......................................... */
    ...
    int tcp_server_up ( void )
    {
      ...
      while (!the_end)
      {
        debug_info("[MPI-SERVER] tcp_server_accept_comm()\n") ;
        ...
        if (ret == -1) {
          printf("[MPI-SERVER] ERROR: tcp_server_comm_readdata fail\n") ;
          return -1;
        }
      ...
      debug_info("[MPI-SERVER] tcp_server_comm_destroy\n");
    ...
    int tcp_server_down ( int argc, char *argv[] )
    {
      ...
      while (fscanf(file, "%[^\n] ", srv_name) != EOF)
      {
        ...
        ret = int_connect( port_name, TCP_INFO_NULL, 0, TCP_COMM_SELF, &server );
        if (ret != 0) {
