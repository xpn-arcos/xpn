 ...
 *    Nothing.
 */
void tcp_server_op_fstat(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head,
 ...
 *    Nothing.
 */
void tcp_server_op_getid(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head,
...
int tcp_server_do_operation(struct st_th *th, int *the_end) {
    ...
    switch (th->type_op) {
        // File API
        case TCP_SERVER_OPEN_FILE_WS:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_open),
                                            sizeof(struct st_tcp_server_open), 0 /*head.id*/);
            if (ret != -1) {
        ...
        case TCP_SERVER_OPEN_FILE_WOS:
            ...
            break;
        case TCP_SERVER_CREAT_FILE_WS:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_creat),
                                            sizeof(struct st_tcp_server_creat), 0 /*head.id*/);
            if (ret != -1) {
        ...
        case TCP_SERVER_CREAT_FILE_WOS:
            ...
            break;
        case TCP_SERVER_READ_FILE_WS:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_read),
                                            sizeof(struct st_tcp_server_read), 0 /*head.id*/);
            if (ret != -1) {
        ...
        case TCP_SERVER_READ_FILE_WOS:
            ...
            break;
        case TCP_SERVER_WRITE_FILE_WS:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_write),
                                            sizeof(struct st_tcp_server_write), 0 /*head.id*/);
            if (ret != -1) {
        ...
        case TCP_SERVER_WRITE_FILE_WOS:
            ...
            break;
        case TCP_SERVER_CLOSE_FILE_WS:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_close),
                                            sizeof(struct st_tcp_server_close), 0 /*head.id*/);
            if (ret != -1) {
        ...
        // Metadata API
        case TCP_SERVER_RM_FILE:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_rm),
                                            sizeof(struct st_tcp_server_rm), 0 /*head.id*/);
            if (ret != -1) {
                tcp_server_op_rm(th->params, (int)th->sd, &head, 0 /*head.id*/);
            }
            break;
        case TCP_SERVER_RENAME_FILE:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_rename),
                                            sizeof(struct st_tcp_server_rename), 0 /*head.id*/);
            if (ret != -1) {
                tcp_server_op_rename(th->params, (int)th->sd, &head, 0 /*head.id*/);
            }
            break;
        case TCP_SERVER_GETATTR_FILE:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_getattr),
                                            sizeof(struct st_tcp_server_getattr), 0 /*head.id*/);
            if (ret != -1) {
                tcp_server_op_getattr(th->params, (int)th->sd, &head, 0 /*head.id*/);
            }
            break;
        case TCP_SERVER_SETATTR_FILE:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_setattr),
                                            sizeof(struct st_tcp_server_setattr), 0 /*head.id*/);
            if (ret != -1) {
        ...
        // Directory API
        case TCP_SERVER_MKDIR_DIR:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_mkdir),
                                            sizeof(struct st_tcp_server_mkdir), 0 /*head.id*/);
            if (ret != -1) {
                tcp_server_op_mkdir(th->params, (int)th->sd, &head, 0 /*head.id*/);
            }
            break;
        case TCP_SERVER_OPENDIR_DIR:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_opendir),
                                            sizeof(struct st_tcp_server_opendir), 0 /*head.id*/);
            if (ret != -1) {
                tcp_server_op_opendir(th->params, (int)th->sd, &head, 0 /*head.id*/);
            }
            break;
        case TCP_SERVER_READDIR_DIR:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_readdir),
                                            sizeof(struct st_tcp_server_readdir), 0 /*head.id*/);
            if (ret != -1) {
                tcp_server_op_readdir(th->params, (int)th->sd, &head, 0 /*head.id*/);
            }
            break;
        case TCP_SERVER_CLOSEDIR_DIR:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_closedir),
                                            sizeof(struct st_tcp_server_closedir), 0 /*head.id*/);
            if (ret != -1) {
                tcp_server_op_closedir(th->params, (int)th->sd, &head, 0 /*head.id*/);
            }
            break;
        case TCP_SERVER_RMDIR_DIR:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_rmdir),
                                            sizeof(struct st_tcp_server_rmdir), 0 /*head.id*/);
            if (ret != -1) {
        ...
        // File system API
        case TCP_SERVER_PRELOAD_FILE:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_preload),
                                            sizeof(struct st_tcp_server_preload), 0 /*head.id*/);
            if (ret != -1) {
                tcp_server_op_preload(th->params, (int)th->sd, &head, 0 /*head.id*/);
            }
            break;
        case TCP_SERVER_FLUSH_FILE:
            ret = tcp_server_comm_read_data(th->params, (int)th->sd, (char *)&(head.u_st_tcp_server_msg.op_flush),
                                            sizeof(struct st_tcp_server_flush), 0 /*head.id*/);
            if (ret != -1) {
...
void tcp_server_op_open_ws(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head,
                           int rank_client_id)  // WS - With Session
{
    int fd;
    char path[PATH_MAX];
    ...
    // show debug info
    debug_info("[%d][TCP-SERVER-OPS] (ID=%s) OPEN(%s)=%d\n", __LINE__, params->srv_name,
               head->u_st_tcp_server_msg.op_open.path, fd);
...
void tcp_server_op_open_wos(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head,
                            int rank_client_id)  // WOS - Without Session
{
    int fd;
    char path[PATH_MAX];
...
void tcp_server_op_creat_ws(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head, int rank_client_id) {
    ...
    debug_info("[%d][TCP-SERVER-OPS] (ID=%s) CREAT(%s)=%d\n", __LINE__, params->srv_name,
               head->u_st_tcp_server_msg.op_creat.path, fd);
...
void tcp_server_op_read_ws(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head, int rank_client_id) {
    ...
    debug_info("[TCP-SERVER-OPS] (ID=%s) begin read: fd %d offset %d size %d ID=x\n", params->srv_name,
               head->u_st_tcp_server_msg.op_read.fd, (int)head->u_st_tcp_server_msg.op_read.offset,
    ...
    do {
        ...
        // lseek and read data...
        filesystem_lseek(head->u_st_tcp_server_msg.op_read.fd, head->u_st_tcp_server_msg.op_read.offset + cont,
                         SEEK_SET);
        ...
        if (req.size < 0) {
            ...
            tcp_server_comm_write_data(params, sd, (char *)&req, sizeof(struct st_tcp_server_write_req),
                                       rank_client_id);
    ...
    debug_info("[TCP-SERVER-OPS] (ID=%s) end READ: fd %d offset %d size %d ID=x\n", params->srv_name,
               head->u_st_tcp_server_msg.op_read.fd, (int)head->u_st_tcp_server_msg.op_read.offset, size);
}
...
void tcp_server_op_read_wos(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head, int rank_client_id) {
    ...
    debug_info("[TCP-SERVER-OPS] (ID=%s) begin read: path %s offset %d size %d ID=x\n", params->srv_name,
               head->u_st_tcp_server_msg.op_read.path, (int)head->u_st_tcp_server_msg.op_read.offset,
               head->u_st_tcp_server_msg.op_read.size);
    ...
    debug_info("[TCP-SERVER-OPS] (ID=%s) end READ: path %s offset %d size %d ID=x\n", params->srv_name,
               head->u_st_tcp_server_msg.op_read.path, (int)head->u_st_tcp_server_msg.op_read.offset, size);
}
...
void tcp_server_op_write_ws(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head, int rank_client_id) {
    ...
    debug_info("[TCP-SERVER-OPS] (ID=%s) begin write: fd %d ID=xn", params->srv_name,
               head->u_st_tcp_server_msg.op_write.fd);
    ...
    do {
        ...
        filesystem_lseek(head->u_st_tcp_server_msg.op_write.fd, head->u_st_tcp_server_msg.op_write.offset + cont,
                         SEEK_SET);
        // sem_wait(&disk_sem);
    ...
    debug_info("[TCP-SERVER-OPS] (ID=%s) end write: fd %d ID=xn", params->srv_name,
               head->u_st_tcp_server_msg.op_write.fd);
}
...
void tcp_server_op_write_wos(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head, int rank_client_id) {
    if (params->mosquitto_mode == 0) {
        ...
        debug_info("[TCP-SERVER-OPS] (ID=%s) begin write: path %s ID=xn", params->srv_name,
                   head->u_st_tcp_server_msg.op_write.path);
...
void tcp_server_op_close_ws(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head, int rank_client_id) {
    ...
    debug_info("[TCP-SERVER-OPS] (ID=%s) CLOSE(fd=%d, path=%s)\n", params->srv_name,
               head->u_st_tcp_server_msg.op_close.fd, head->u_st_tcp_server_msg.op_close.path);
...
void tcp_server_op_setattr(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head,
...
void tcp_server_op_opendir(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head, int rank_client_id) {
    ...
    tcp_server_comm_write_data(params, sd, (char *)(unsigned long long *)&(ret), (unsigned int)sizeof(DIR *),
                               rank_client_id);
...
void tcp_server_op_preload(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head, int rank_client_id) {
    ...
    if (fd_orig < 0) {
        tcp_server_comm_write_data(params, sd, (char *)&fd_orig, sizeof(int),
                                   rank_client_id);  // TO-DO: Check error treatment client-side
        return;
    }

    ret = base_urlstr_parse_url(head->u_st_tcp_server_msg.op_preload.virtual_path, protocol, user, machine, port, file,
                                params1);
    ...
    if (fd_dest < 0) {
        ...
        tcp_server_comm_write_data(params, sd, (char *)&fd_dest, sizeof(int),
                                   rank_client_id);  // TO-DO: Check error treatment client-side
        return;
    }
    ...
    do {
        ...
        if (ret_2 < (off_t)-1) {
            ...
            tcp_server_comm_write_data(params, sd, (char *)&ret, sizeof(int),
                                       rank_client_id);  // TO-DO: Check error treatment client-side
            return;
        }
        ...
        if (read_bytes < 0) {
            filesystem_close(fd_orig);
            filesystem_close(fd_dest);
            tcp_server_comm_write_data(params, sd, (char *)&read_bytes, sizeof(int),
                                       rank_client_id);  // TO-DO: Check error treatment client-side
            return;
        }
        ...
        if (read_bytes > 0) {
            ...
            if (write_bytes < 0) {
                filesystem_close(fd_orig);
                filesystem_close(fd_dest);
                tcp_server_comm_write_data(params, sd, (char *)&write_bytes, sizeof(int),
                                           rank_client_id);  // TO-DO: Check error treatment client-side
                return;
            }
        }
    ...
    debug_info("[TCP-SERVER-OPS] (ID=%s) PRELOAD(%s,%s)->%d\n", params->srv_name,
               head->u_st_tcp_server_msg.op_preload.virtual_path, head->u_st_tcp_server_msg.op_preload.storage_path,
               ret);
}
...
void tcp_server_op_flush(tcp_server_param_st *params, int sd, struct st_tcp_server_msg *head, int rank_client_id) {
    ...
    ret = base_urlstr_parse_url(head->u_st_tcp_server_msg.op_flush.virtual_path, protocol, user, machine, port, file,
                                params1);
 ...
 ***********************************************/
void tcp_server_op_getnodename(tcp_server_param_st *params, int sd,
                               __attribute__((__unused__)) struct st_tcp_server_msg *head, int rank_client_id) {
    ...
    tcp_server_comm_write_data(params, sd, (char *)serv_name, HOST_NAME_MAX,
                               rank_client_id);  // Send one single message
    tcp_server_comm_write_data(params, sd, (char *)params->dirbase, PATH_MAX,
                               rank_client_id);  // Send one single message
    // tcp_server_comm_write_data(params, sd, (char * ) params->sem_name_server, PATH_MAX, rank_client_id); // Send one
    // single message
