 ...
 */
  ...
  /* ... Functions / Funciones ......................................... */
     ...
     void XpnShowFileTable(void)
     {
         ...
         printf("<file_table %d>\n", XPN_MAX_FILE);
         while ((i < XPN_MAX_FILE) && (xpn_file_table[i] != NULL))
         {
     ...
     int XpnSearchFile ( const char * path )
     {
         ...
         {
             if ((xpn_file_table[i] != NULL) && (strcmp(xpn_file_table[i]->path, path) == 0))
             {
                 break;
             }
     ...
     int xpn_simple_dup(int fd)
     {
         ...
         // check duplicate descriptor and return descriptor
