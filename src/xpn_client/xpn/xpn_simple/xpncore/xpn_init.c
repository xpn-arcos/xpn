...
int xpn_init_partition(__attribute__((__unused__)) char * partname) {
    ...
    while ((res = XpnGetNextPartition(fd, xpn_parttable[i].name)) > 0) {
        ...
        for (j = 0; j < xpn_parttable[i].data_nserv; j++) {
            ...
            if (res < 0) {
                ...
                fprintf(stderr, "xpn_init: Data configuration incorrect(1)\n");
        ...
        printf("Partition %d end", xpn_parttable[i].id);
    ...
    if (res < 0) {
        ...
        fprintf(stderr, "xpn_init: Data configuration incorrect(2).\n");
