...
int xpn_init_partition ( void )
{
    ...
    for (i=0; i < part_n; i++)
    {
      ...
      if ( (res != 0) || (xpn_parttable[i].block_size == 1) ) {
            xpn_parttable[i].block_size = XPN_CONF_DEFAULT_BLOCKSIZE;
      }
      ...
      if ( (res != 0) || (xpn_parttable[i].replication_level < 0) ) {
            xpn_parttable[i].replication_level = XPN_CONF_DEFAULT_REPLICATION_LEVEL;
      }
