...
int XpnGetEntry(int fd, char *entry, unsigned char *type)
{
	...
	/* params:
     * flag operation , partition id,absolute path, file descript.,
	 * pointer to server
	 */
	servers = NULL;
	...
	res = xpn_file_table[fd]->data_vfh->nfih[0]->server->ops->nfi_readdir(
		xpn_file_table[fd]->data_vfh->nfih[0]->server,
		xpn_file_table[fd]->data_vfh->nfih[0],
		entry,
		type);
