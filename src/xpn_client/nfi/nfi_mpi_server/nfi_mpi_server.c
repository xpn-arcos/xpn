    ...
    switch (head->type) {
        ...
        case MPI_SERVER_OPEN_FILE_WOS:
            debug_info("[NFI-MPI] (ID=%s) OPEN operation ---------------------  %ld\n", head->id,
                       sizeof(struct st_mpi_server_open));
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_open,
                                       sizeof(struct st_mpi_server_open), head->id);
            break;
        case MPI_SERVER_CREAT_FILE_WS:
        case MPI_SERVER_CREAT_FILE_WOS:
            debug_info("[NFI-MPI] (ID=%s) CREAT operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_creat,
                                       sizeof(struct st_mpi_server_creat), head->id);
            break;
        case MPI_SERVER_READ_FILE_WS:
        case MPI_SERVER_READ_FILE_WOS:
            debug_info("[NFI-MPI] (ID=%s) READ operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_read,
                                       sizeof(struct st_mpi_server_read), head->id);
            break;
        case MPI_SERVER_WRITE_FILE_WS:
        case MPI_SERVER_WRITE_FILE_WOS:
            debug_info("[NFI-MPI] (ID=%s) WRITE operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_write,
                                       sizeof(struct st_mpi_server_write), head->id);
            break;
        case MPI_SERVER_CLOSE_FILE_WS:
            debug_info("[NFI-MPI] (ID=%s) CLOSE operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_close,
                                       sizeof(struct st_mpi_server_close), head->id);
            break;
        case MPI_SERVER_RM_FILE:
            debug_info("[NFI-MPI] (ID=%s) RM operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_rm, sizeof(struct st_mpi_server_rm),
                                       head->id);
            break;
        ...
        case MPI_SERVER_RENAME_FILE:
            ...
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_rename,
                                       sizeof(struct st_mpi_server_rename), head->id);
            break;
        case MPI_SERVER_GETATTR_FILE:
            debug_info("[NFI-MPI] (ID=%s) GETATTR operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_getattr,
                                       sizeof(struct st_mpi_server_getattr), head->id);
            break;

        // Directory API
        case MPI_SERVER_MKDIR_DIR:
            debug_info("[NFI-MPI] (ID=%s) MDKIR operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_mkdir,
                                       sizeof(struct st_mpi_server_mkdir), head->id);
            break;
        case MPI_SERVER_OPENDIR_DIR:
            debug_info("[NFI-MPI] (ID=%s) OPENDIR operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_opendir,
                                       sizeof(struct st_mpi_server_opendir), head->id);
            break;
        case MPI_SERVER_READDIR_DIR:
            debug_info("[NFI-MPI] (ID=%s) READDIR operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_readdir,
                                       sizeof(struct st_mpi_server_readdir), head->id);
            break;
        case MPI_SERVER_CLOSEDIR_DIR:
            debug_info("[NFI-MPI] (ID=%s) CLOSEDIR operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_closedir,
                                       sizeof(struct st_mpi_server_closedir), head->id);
            break;
        case MPI_SERVER_RMDIR_DIR:
            debug_info("[NFI-MPI] (ID=%s) RMDIR operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_rmdir,
                                       sizeof(struct st_mpi_server_rmdir), head->id);
            break;
        ...
        case MPI_SERVER_FLUSH_FILE:
            ...
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_flush,
                                       sizeof(struct st_mpi_server_flush), head->id);
            break;
        case MPI_SERVER_PRELOAD_FILE:
            debug_info("[NFI-MPI] (ID=%s) PRELOAD operation\n", head->id);
            ret = mpiClient_write_data(sd, (char *)&head->u_st_mpi_server_msg.op_preload,
                                       sizeof(struct st_mpi_server_preload), head->id);
            break;
    }
...
int nfi_mpi_server_doRequest(struct nfi_mpi_server_server *server_aux, struct st_mpi_server_msg *msg, char *req,
                             int req_size) {
    ssize_t ret;
...
/************************************************
 *  ... Private Functions to use MPI_SERVER
 ***********************************************/
 ...
 * @return 'TODO'.
 */
void nfi_to_mpi_server_attr(struct stat *att, struct nfi_attr *nfi_att) {
 ...
 * @return 'TODO'.
 */
void mpi_server_to_nfi_attr(struct nfi_attr *nfi_att, struct stat *att) {
 ...
 *
 * @param nfi_inf 'TODO'.
 * @param mpi_server_inf 'TODO'.
 * @return 'TODO'.
 */
void mpi_server_to_nfi_info(__attribute__((__unused__)) struct nfi_info *nfi_inf,
                            __attribute__((__unused__)) struct nfi_info *mpi_server_inf) {
    // TODO
}
...
int nfi_mpi_server_connect(struct nfi_server *serv, __attribute__((__unused__)) char *url,
                           __attribute__((__unused__)) char *prt, char *server, __attribute__((__unused__)) char *dir) {
    int ret;
    struct nfi_mpi_server_server *server_aux;
...
int nfi_mpi_server_create(struct nfi_server *serv, char *url, struct nfi_attr *attr, struct nfi_fhandle *fh) {
    ...
    mpi_server_to_nfi_attr(attr, &req.attr);
...
ssize_t nfi_mpi_server_read(struct nfi_server *serv, struct nfi_fhandle *fh, void *buffer, off_t offset, size_t size) {
    ...
    if (server_aux->params.locality) {
        if (server_aux->params.xpn_session) {
            ...
            debug_info("[NFI-MPI] read %s(%d) off %ld size %zu (ret:%zd)", fh->url, fh_aux->fd, (long int)offset, size,
                       ret) if (ret < 0) {
            ...
            debug_info("[NFI-MPI] read %s(%d) off %ld size %zu (ret:%zd)", fh->url, fd, (long int)offset, size,
                       ret) if (ret < 0) {
                debug_error("real_posix_read reads zero bytes from url:%s offset:%ld size:%zu (ret:%zd) errno=%d\n",
                            fh->url, (long int)offset, size, ret, errno);
                return -1;
    ...
    /************** REMOTE ****************/
        ...
        do {
            ret = mpiClient_read_data(server_aux->params.server, (char *)&req, sizeof(struct st_mpi_server_read_req),
                                      msg.id);
...
ssize_t nfi_mpi_server_write(struct nfi_server *serv, struct nfi_fhandle *fh, void *buffer, off_t offset, size_t size) {
    ...
    if (server_aux->params.locality) {
        if (server_aux->params.xpn_session) {
            ...
            debug_info("[NFI-MPI] write %s(%d) off %ld size %zu (ret:%zd)", fh->url, fh_aux->fd, (long int)offset, size,
                       ret);
            // if(server_aux->params.sem_server != 0) sem_post(server_aux->params.sem_server);
    ...
    /************** REMOTE ****************/
        ...
        ret = mpiClient_read_data(server_aux->params.server, (char *)&req, sizeof(struct st_mpi_server_write_req),
                                  msg.id);
        if (ret < 0) {
...
int nfi_mpi_server_close(struct nfi_server *serv, struct nfi_fhandle *fh) {
    ...
    /************** REMOTE ****************/
        ...
        debug_info("[NFI-MPI] nfi_mpi_server_close(ID=%s): close -> %d \n", server_aux->id,
                   msg.u_st_mpi_server_msg.op_close.fd);
    }
...
int nfi_mpi_server_statfs(__attribute__((__unused__)) struct nfi_server *serv,
                          __attribute__((__unused__)) struct nfi_info *inf) {
    DEBUG_BEGIN();
