...
void mpi_server_to_nfi_info(__attribute__((__unused__)) struct nfi_info *nfi_inf, __attribute__((__unused__)) struct nfi_info *mpi_server_inf)
{
  // TODO
}
...
ssize_t nfi_mpi_server_read(struct nfi_server *serv, struct nfi_fhandle *fh, void *buffer, off_t offset, size_t size)
{
  ...
  if (server_aux->params.locality)
  {
    if (server_aux->params.xpn_session)
    {
      ...
      debug_info("[NFI-MPI] read %s(%d) off %ld size %zu (ret:%zd)", fh->url, fh_aux->fd, (long int)offset, size, ret) if (ret < 0)
      {
    ...
    else
    {
      ...
      debug_info("[NFI-MPI] read %s(%d) off %ld size %zu (ret:%zd)", fh->url, fd, (long int)offset, size, ret) if (ret < 0)
      {
        debug_error("real_posix_read reads zero bytes from url:%s offset:%ld size:%zu (ret:%zd) errno=%d\n", fh->url, (long int)offset, size, ret, errno);
        return -1;
      }
