...
void *nfi_worker_run ( struct nfi_worker *args )
{
  ...
  while(is_true)
  {
    ...
    debug_info("[NFI_WORKER] nfi_worker_run(%lu) op: %d",(unsigned long int)pthread_self(),wrk->arg.operation)

    switch (wrk->arg.operation)
    {
      case op_getattr:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_getattr",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_getattr(wrk->server, wrk->arg.fh, wrk->arg.attr) ;
        break;
      case op_setattr:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_setattr",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_setattr(wrk->server, wrk->arg.fh, wrk->arg.attr) ;
        break;
      case op_preload:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_preload",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_preload(wrk->server, wrk->arg.url, wrk->arg.virtual_path, wrk->arg.storage_path, wrk->arg.opt) ;
        break;
      case op_flush:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_flush",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_flush(wrk->server, wrk->arg.url, wrk->arg.virtual_path, wrk->arg.storage_path, wrk->arg.opt) ;
        break;
      case op_open:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_open",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_open(wrk->server, wrk->arg.url, wrk->arg.fh) ;
        break;
      case op_create:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_create",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_create(wrk->server, wrk->arg.url, wrk->arg.attr, wrk->arg.fh) ;
        break;
      case op_close:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_close",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_close(wrk->server, wrk->arg.fh) ;
        break;
      case op_remove:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_remove",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_remove(wrk->server, wrk->arg.url) ;
        break;
      case op_rename:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_rename",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_rename(wrk->server, wrk->arg.url, wrk->arg.newurl) ;
        break;
      case op_read:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_read",(unsigned long int)pthread_self())
        ret = 0;
        ...
        break;
      case op_write:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_write",(unsigned long int)pthread_self())
        ret = 0;
        for(i=0; i<wrk->arg.n_io;i++)
        {
        ...
        break;
      case op_mkdir:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_mkdir",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_mkdir(wrk->server, wrk->arg.url, wrk->arg.attr, wrk->arg.fh) ;
        break;
      case op_rmdir:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_rmdir",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_rmdir(wrk->server, wrk->arg.url) ;
        break;
      case op_opendir:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_opendir",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_opendir(wrk->server, wrk->arg.url, wrk->arg.fh) ;
        break;
      case op_readdir:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_readdir",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_readdir(wrk->server, wrk->arg.fh, wrk->arg.entry, wrk->arg.type) ;
        break;
      case op_closedir:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_closedir",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_closedir(wrk->server, wrk->arg.fh) ;
        break;
      case op_statfs:
        debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> nfi_statfs",(unsigned long int)pthread_self())
        ret = wrk->server->ops->nfi_statfs(wrk->server, wrk->arg.inf) ;
        break;
    }

    debug_info("[NFI_WORKER] nfi_worker_run(%lu) -> end op: %d",(unsigned long int)pthread_self(),wrk->arg.operation)
...
int nfi_worker_do_getattr(struct nfi_worker *wrk, struct nfi_fhandle *fh, struct nfi_attr *attr)
{
  ...
  if(wrk->thread)
  {
    debug_info("[NFI_WORKER] nfi_worker_do_getattr(%lu) with_threads",(unsigned long int)pthread_self())
  ...
  {
    debug_info("[NFI_WORKER] nfi_worker_do_getattr(%lu) without_threads",(unsigned long int)pthread_self())
    wrk->arg.result = wrk->server->ops->nfi_getattr(wrk->server, wrk->arg.fh, wrk->arg.attr) ;
  }
...
int nfi_worker_do_flush(struct nfi_worker *wrk, char *url, char *virtual_path, char *storage_path, int opt)
{
  ...
  {
    wrk->arg.result = wrk->server->ops->nfi_flush(wrk->server, wrk->arg.url, wrk->arg.virtual_path, wrk->arg.storage_path, wrk->arg.opt) ;
  }
...
int nfi_worker_do_preload(struct nfi_worker *wrk, char *url, char *virtual_path, char *storage_path, int opt)
{
  ...
  if(wrk->thread)
  {
    debug_info("[NFI_WORKER] nfi_worker_do_preload(%lu) with_threads",(unsigned long int)pthread_self())
    /* wake up thread */
    wrk->ready = 1;
    pthread_mutex_lock(&(wrk->mt)) ;
    if (wrk->ready){
  ...
  {
    debug_info("[NFI_WORKER] nfi_worker_do_preload(%lu) without_threads",(unsigned long int)pthread_self())
    wrk->arg.result = wrk->server->ops->nfi_preload(wrk->server, wrk->arg.url, wrk->arg.virtual_path, wrk->arg.storage_path, wrk->arg.opt) ;
  }
...
int nfi_worker_do_open(struct nfi_worker *wrk, char *url, struct nfi_fhandle *fh)
{
  ...
  if(wrk->thread)
  {
    debug_info("[NFI_WORKER] nfi_worker_do_open(%lu) with_threads",(unsigned long int)pthread_self())
    /* wake up thread */
    wrk->ready = 1;
    pthread_mutex_lock(&(wrk->mt)) ;
    if (wrk->ready){
  ...
  {
    debug_info("[NFI_WORKER] nfi_worker_do_open(%lu) without_threads",(unsigned long int)pthread_self())
    wrk->arg.result = wrk->server->ops->nfi_open(wrk->server, wrk->arg.url, wrk->arg.fh) ;
  }
...
int nfi_worker_do_create(struct nfi_worker *wrk, char *url, struct nfi_attr *attr, struct nfi_fhandle  *fh)
{
  ...
  if(wrk->thread)
  {
    debug_info("[NFI_WORKER] nfi_worker_do_create(%lu) with_threads",(unsigned long int)pthread_self())
    /* wake up thread */
    wrk->ready = 1;
    pthread_mutex_lock(&(wrk->mt)) ;
    if (wrk->ready){
    ...
    debug_info("[NFI_WORKER] nfi_worker_do_create(%lu) without_threads",(unsigned long int)pthread_self())
    wrk->arg.result = wrk->server->ops->nfi_create(wrk->server, wrk->arg.url, wrk->arg.attr, wrk->arg.fh) ;
  }
...
int nfi_worker_do_close(struct nfi_worker *wrk, struct nfi_fhandle *fh)
{
  ...
  if(wrk->thread)
  {
    debug_info("[NFI_WORKER] nfi_worker_do_close(%lu) with_threads",(unsigned long int)pthread_self())
    /* wake up thread */
    wrk->ready = 1;
    pthread_mutex_lock(&(wrk->mt)) ;
    if (wrk->ready){
  ...
  {
    debug_info("[NFI_WORKER] nfi_worker_do_close(%lu) without_threads",(unsigned long int)pthread_self())
    wrk->arg.result = wrk->server->ops->nfi_close(wrk->server,wrk->arg.fh) ;
  }
...
int nfi_worker_do_remove(struct nfi_worker *wrk, char *url)
{
  ...
  if(wrk->thread)
  {
    debug_info("[NFI_WORKER] nfi_worker_do_remove(%lu) with_threads",(unsigned long int)pthread_self())
    ...
    debug_info("[NFI_WORKER] nfi_worker_do_remove(%lu) without_threads",(unsigned long int)pthread_self())
    wrk->arg.result = wrk->server->ops->nfi_remove(wrk->server, wrk->arg.url) ;
  }
...
int nfi_worker_do_rename(struct nfi_worker *wrk, char *old_url, char *new_url)
{
  ...
  {
    debug_info("[NFI_WORKER] nfi_worker_do_remove(%lu) without_threads",(unsigned long int)pthread_self())
    wrk->arg.result = wrk->server->ops->nfi_rename(wrk->server, wrk->arg.url, wrk->arg.newurl) ;
  }
...
int nfi_worker_do_read(struct nfi_worker *wrk, struct nfi_fhandle *fh, struct nfi_worker_io *io, int n)
{
  ...
  if(wrk->thread)
  {
    debug_info("[NFI_WORKER] nfi_worker_do_read(%lu) with_threads",(unsigned long int)pthread_self())
    /* wake up thread */
    pthread_mutex_lock(&(wrk->mt)) ;
    ...
    debug_info("[NFI_WORKER] nfi_worker_do_read(%lu) without_threads",(unsigned long int)pthread_self())
...
int nfi_worker_do_write(struct nfi_worker *wrk, struct nfi_fhandle *fh, struct nfi_worker_io *io, int n)
{
  ...
  if(wrk->thread)
  {
    debug_info("[NFI_WORKER] nfi_worker_do_write(%lu) with_threads",(unsigned long int)pthread_self())
    /* wake up thread */
    pthread_mutex_lock(&(wrk->mt)) ;
    ...
    debug_info("[NFI_WORKER] nfi_worker_do_write(%lu) without_threads",(unsigned long int)pthread_self())
...
int nfi_worker_do_mkdir(struct nfi_worker *wrk, char *url, struct nfi_attr *attr, struct nfi_fhandle *fh)
{
  ...
  {
    wrk->arg.result = wrk->server->ops->nfi_mkdir(wrk->server, wrk->arg.url, wrk->arg.attr, wrk->arg.fh) ;
  }
...
int nfi_worker_do_rmdir(struct nfi_worker *wrk, char *url)
{
  ...
  if(wrk->thread)
  {
    ...
    wrk->arg.result= wrk->server->ops->nfi_rmdir(wrk->server, wrk->arg.url) ;
  }
...
int nfi_worker_do_opendir(struct nfi_worker *wrk, char *url, struct nfi_fhandle *fh)
{
  ...
  {
    wrk->arg.result = wrk->server->ops->nfi_opendir(wrk->server, wrk->arg.url, wrk->arg.fh) ;
  }
...
int nfi_worker_do_readdir(struct nfi_worker *wrk, struct nfi_fhandle *fh, char *entry, unsigned char *type)
{
  ...
  if(wrk->thread){
    ...
    wrk->arg.result = wrk->server->ops->nfi_readdir(wrk->server, wrk->arg.fh, wrk->arg.entry, wrk->arg.type) ;
  }
