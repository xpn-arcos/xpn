...
int sck_server_write_operation(int sd, struct st_sck_server_msg * head) {
  ...
  debug_info("[NFI-SCK-CLI] (ID=%s) sckClient_write_data: begin               HEAD_TYPE:%d\n", head->id, sizeof(head->type));
  ret = sckClient_write_operation(sd, (char * ) & (head->type), 1, head->id);
  debug_info("CLIENT -- %d\n", head->type);
  if (ret < 0) {
    debug_warning("Server[?]: sckClient_write_data fails :-(");
    return -1;
  }

  debug_info("[NFI-SCK-CLI] (ID=%s) write_operation: %d->\n", head->id, head->type);
  switch (head->type) {
    //File API
  case SCK_SERVER_OPEN_FILE_WS:
  case SCK_SERVER_OPEN_FILE_WOS:
    debug_info("[NFI-SCK-CLI] (ID=%s) OPEN operation ---------------------  %ld\n", head->id, sizeof(struct st_sck_server_open));
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_open, sizeof(struct st_sck_server_open), head->id);
    break;
  case SCK_SERVER_CREAT_FILE_WS:
  case SCK_SERVER_CREAT_FILE_WOS:
    debug_info("[NFI-SCK-CLI] (ID=%s) CREAT operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_creat, sizeof(struct st_sck_server_creat), head->id);
    break;
  case SCK_SERVER_READ_FILE_WS:
  case SCK_SERVER_READ_FILE_WOS:
    debug_info("[NFI-SCK-CLI] (ID=%s) READ operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_read, sizeof(struct st_sck_server_read), head->id);
    break;
  case SCK_SERVER_WRITE_FILE_WS:
  case SCK_SERVER_WRITE_FILE_WOS:
    debug_info("[NFI-SCK-CLI] (ID=%s) WRITE operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_write, sizeof(struct st_sck_server_write), head->id);
    break;
  case SCK_SERVER_CLOSE_FILE_WS:
    debug_info("[NFI-SCK-CLI] (ID=%s) CLOSE operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_close, sizeof(struct st_sck_server_close), head->id);
    break;
  case SCK_SERVER_RM_FILE:
    debug_info("[NFI-SCK-CLI] (ID=%s) RM operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_rm, sizeof(struct st_sck_server_rm), head->id);
    break;
  case SCK_SERVER_RENAME_FILE:
    debug_info("[NFI-SCK-CLI] (ID=%s) RENAME operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_rename, sizeof(struct st_sck_server_rename), head->id);
    break;
  case SCK_SERVER_GETATTR_FILE:
    debug_info("[NFI-SCK-CLI] (ID=%s) GETATTR operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_getattr, sizeof(struct st_sck_server_getattr), head->id);
    break;

    //Directory API
  case SCK_SERVER_MKDIR_DIR:
    debug_info("[NFI-SCK-CLI] (ID=%s) MKDIR operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_mkdir, sizeof(struct st_sck_server_mkdir), head->id);
    break;
  case SCK_SERVER_OPENDIR_DIR:
    debug_info("[NFI-SCK-CLI] (ID=%s) OPENDIR operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_opendir, sizeof(struct st_sck_server_opendir), head->id);
    break;
  case SCK_SERVER_READDIR_DIR:
    debug_info("[NFI-SCK-CLI] (ID=%s) READDIR operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_readdir, sizeof(struct st_sck_server_readdir), head->id);
    break;
  case SCK_SERVER_CLOSEDIR_DIR:
    debug_info("[NFI-SCK-CLI] (ID=%s) CLOSEDIR operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_closedir, sizeof(struct st_sck_server_closedir), head->id);
    break;
  case SCK_SERVER_RMDIR_DIR:
    debug_info("[NFI-SCK-CLI] (ID=%s) RMDIR operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_rmdir, sizeof(struct st_sck_server_rmdir), head->id);
    break;

    //Optimization API
  case SCK_SERVER_FLUSH_FILE:
    debug_info("[NFI-SCK-CLI] (ID=%s) FLUSH operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_flush, sizeof(struct st_sck_server_flush), head->id);
    break;
  case SCK_SERVER_PRELOAD_FILE:
    debug_info("[NFI-SCK-CLI] (ID=%s) PRELOAD operation\n", head->id);
    ret = sckClient_write_data(sd, (char * ) & head->u_st_sck_server_msg.op_preload, sizeof(struct st_sck_server_preload), head->id);
    break;
  }
...
int nfi_sck_server_doRequest(struct nfi_sck_server_server * server_aux, struct st_sck_server_msg * msg, char * req, int req_size) {
  ...
  // read response...
  debug_info("[NFI-SCK-CLI] (ID=%s): %s: <- ...\n", server_aux->id, msg->id);
  ret = sckClient_read_data(server_aux->params.server, req, req_size, msg->id);
  if (ret < 0) {
    return -1;
  }
...
/*
 *  PRIVATE FUNCTIONS TO USE sck_server SERVERS
 */

void NFItoSCK_SERVERattr(struct stat * att, struct nfi_attr * nfi_att) {
...
void SCK_SERVERtoNFIattr(struct nfi_attr * nfi_att, struct stat * att) {
...
/*
 * PUBLIC FUNCTIONS TO USE SCK_SERVER
 */
...
int nfi_sck_server_init(char * url, struct nfi_server * serv, __attribute__((__unused__)) struct nfi_attr_server * attr) {
  ...
  NULL_RET_ERR(serv->ops, SCK_SERVERERR_MEMORY);
  ...
  if (ret < 0) {
    sck_server_err(SCK_SERVERERR_URL);
    debug_error("nfi_sck_server_init: incorrect url '%s'.\n", url);
  ...
  // Initialize params
  memset( & (server_aux->params), 0, sizeof(sckClient_param_st));
  ...
  // initialize socket (TCP) Client communication side...
  ret = sckClient_comm_init( & (server_aux->params));
  if (ret < 0) {
    FREE_AND_NULL(serv->ops);
  ...
  // copy 'url' string...
  serv->url = strdup(url);
  NULL_RET_ERR(serv->url, SCK_SERVERERR_MEMORY);
  ...
  debug_info("[NFI-SCK-CLI] nfiworker_init()\n");
  ...
  debug_info("[%s][%d]\tnfi_sck_server_connect - %d\n", __FILE__, __LINE__, ret);

  ret = sckClient_comm_locality( & (server_aux->params));
  if (ret < 0) {
    FREE_AND_NULL(serv->ops);
    FREE_AND_NULL(server_aux);
    return -1;
  }
  debug_info("[%s][%d]\tsckClient_comm_locality - %d\n", __FILE__, __LINE__, ret);
...
int nfi_sck_server_destroy(struct nfi_server * serv) {
  ...
  // Thread destroy...
  debug_info("[NFI-SCK-CLI] nfiworker_destroy()\n");
  nfiworker_destroy(serv);

  // MPI Finalize...
  ret = sckClient_comm_destroy( & (server_aux->params));
  if (ret < 0) {
    debug_error("[NFI-SCK-CLI]: sckClient_comm_destroy fails :-(");
  }
...
int nfi_sck_server_connect(struct nfi_server * serv, __attribute__((__unused__)) char * url, __attribute__((__unused__)) char * prt, char * server, __attribute__((__unused__)) char * dir) {
  ...
  ret = sckClient_comm_connect( & (server_aux->params));
  if (ret < 0) {
    return -1;
  }
...
int nfi_sck_server_disconnect(struct nfi_server * serv) {
  ...
  // socket (TCP) Disconnect... - ALWAYS RETURNS 0
  ret = sckClient_comm_disconnect( & (server_aux->params));
  if (ret < 0) {
    debug_error("[NFI-SCK-CLI]: sckClient_comm_disconnect fails :-(");
  }
...
/************************************************************
 * Reconnect to the socket (TCP) server                     *
 ************************************************************/
int nfi_sck_server_reconnect(struct nfi_server * serv) {
  ...
  if (ret < 0) {
    sck_server_err(SCK_SERVERERR_URL);
    debug_error("nfi_sck_server_reconnect: url %s incorrect.\n", serv->url);
    return -1;
  }

  // get private_info...
  server_aux = (struct nfi_sck_server_server * ) malloc(sizeof(struct nfi_sck_server_server));
  NULL_RET_ERR(server_aux, SCK_SERVERERR_MEMORY);
  ...
  ret = sckClient_comm_connect( & (server_aux->params));
  if (ret < 0) {
    debug_error("nfi_sck_server_reconnect: sckClient_comm_connect fails.\n");
    return -1;
  }
...
int nfi_sck_server_open(struct nfi_server * serv, char * url, struct nfi_fhandle * fho) {
  ...
  // Check arguments...
  NULL_RET_ERR(serv, SCK_SERVERERR_PARAM);
  NULL_RET_ERR(fho, SCK_SERVERERR_PARAM);
  nfi_sck_server_keepConnected(serv);
  NULL_RET_ERR(serv->private_info, SCK_SERVERERR_PARAM);
  ...
  fho->url = strdup(url);
  NULL_RET_ERR(fho->url, SCK_SERVERERR_MEMORY);
...
int nfi_sck_server_create(struct nfi_server * serv, char * url, struct nfi_attr * attr, struct nfi_fhandle * fh) {
  ...
  // Check arguments...
  NULL_RET_ERR(serv, SCK_SERVERERR_PARAM);
  NULL_RET_ERR(attr, SCK_SERVERERR_PARAM);
  nfi_sck_server_keepConnected(serv);
  NULL_RET_ERR(serv->private_info, SCK_SERVERERR_PARAM);
  ...
  fh_aux = (struct nfi_sck_server_fhandle * ) malloc(sizeof(struct nfi_sck_server_fhandle));
  NULL_RET_ERR(fh_aux, SCK_SERVERERR_MEMORY);
  ...
  SCK_SERVERtoNFIattr(attr, & req.attr);
...
ssize_t nfi_sck_server_read(struct nfi_server * serv, struct nfi_fhandle * fh, void * buffer, off_t offset, size_t size) {
  ...
  // Check arguments...
  NULL_RET_ERR(serv, SCK_SERVERERR_PARAM);
  NULL_RET_ERR(fh, SCK_SERVERERR_PARAM);
  nfi_sck_server_keepConnected(serv);
  NULL_RET_ERR(serv->private_info, SCK_SERVERERR_PARAM);
  ...
  /************** REMOTE ****************/
    ...
    cont = 0;
    do {
      ret = sckClient_read_data(server_aux->params.server, (char * ) & req, sizeof(struct st_sck_server_read_req), msg.id);
      debug_info("[NFI-SCK-CLI] nfi_sck_server_read(ID=%s): (1)sckClient_read_data = %d.\n", server_aux->id, ret);
      ...
      if (req.size > 0) {
        debug_info("[NFI-SCK-CLI] nfi_sck_server_read(ID=%s): (2)sckClient_read_data = %d. size = %d\n", server_aux->id, ret, req.size);
        ret = sckClient_read_data(server_aux->params.server, (char * ) buffer + cont, req.size, msg.id);
        debug_info("[NFI-SCK-CLI] nfi_sck_server_read(ID=%s): (2)sckClient_read_data = %d.\n", server_aux->id, ret);
    ...
    if (req.size < 0) {
      ...
      sck_server_err(SCK_SERVERERR_READ);
      return -1;
    }
...
int nfi_sck_server_remove(struct nfi_server * serv, char * url) {
  ...
  // Check arguments...
  NULL_RET_ERR(serv, SCK_SERVERERR_PARAM);
  NULL_RET_ERR(url, SCK_SERVERERR_PARAM);
  nfi_sck_server_keepConnected(serv);
  NULL_RET_ERR(serv->private_info, SCK_SERVERERR_PARAM);
  ...
  if (server_aux == NULL) {
    sck_server_err(SCK_SERVERERR_PARAM);
    return -1;
  }
...
int nfi_sck_server_rename(struct nfi_server * serv, char * old_url, char * new_url) {
  ...
  // Check arguments...
  NULL_RET_ERR(serv, SCK_SERVERERR_PARAM);
  NULL_RET_ERR(old_url, SCK_SERVERERR_PARAM);
  NULL_RET_ERR(new_url, SCK_SERVERERR_PARAM);
  nfi_sck_server_keepConnected(serv);
  NULL_RET_ERR(serv->private_info, SCK_SERVERERR_PARAM);
...
int nfi_sck_server_setattr(struct nfi_server * serv, struct nfi_fhandle * fh, struct nfi_attr * attr) {
  ...
  NULL_RET_ERR(fh, SCK_SERVERERR_PARAM);
  NULL_RET_ERR(attr, SCK_SERVERERR_PARAM);
  NULL_RET_ERR(fh->priv_fh, SCK_SERVERERR_PARAM);
  nfi_sck_server_keepConnected(serv);
  NULL_RET_ERR(serv->private_info, SCK_SERVERERR_PARAM);
...
int nfi_sck_server_mkdir(struct nfi_server * serv, char * url, struct nfi_attr * attr, struct nfi_fhandle * fh) {
  ...
  /************** SERVER ****************/
    ...
    if ((fh_aux->fd < 0) && (errno != EEXIST)) {
      sck_server_err(SCK_SERVERERR_MKDIR);
...
int nfi_sck_server_readdir(struct nfi_server * serv, struct nfi_fhandle * fh, struct dirent * entry) {
  ...
  if (fh->type != NFIDIR) {
    sck_server_err(SCK_SERVERERR_NOTDIR);
    return -1;
  }
...
int nfi_sck_server_rmdir(struct nfi_server * serv, char * url) {
  ...
  /************** SERVER ****************/
    ...
    if (ret < 0) {
      ...
      sck_server_err(SCK_SERVERERR_REMOVE);
      return -1;
    }
  }
...
int nfi_sck_server_statfs(__attribute__((__unused__)) struct nfi_server * serv, __attribute__((__unused__)) struct nfi_info * inf) {
  ...
  // Check arguments...
  NULL_RET_ERR(serv, SCK_SERVERERR_PARAM) ;
  NULL_RET_ERR(inf,  SCK_SERVERERR_PARAM) ;
  nfi_sck_server_keepConnected(serv) ;
  NULL_RET_ERR(serv->private_info, SCK_SERVERERR_PARAM) ;
  ...
  if (ret <0).{
    sck_server_err(SCK_SERVERERR_STATFS) ;
    return -1;
  }

  MPItoNFIInfo(inf, &sck_serverinf) ;
  */
...
int nfi_sck_server_preload(struct nfi_server * serv, char * url, char * virtual_path, char * storage_path, int opt) {
  ...
  NULL_RET_ERR(serv, SCK_SERVERERR_PARAM);
  NULL_RET_ERR(url, SCK_SERVERERR_PARAM);
  NULL_RET_ERR(virtual_path, SCK_SERVERERR_PARAM);
  NULL_RET_ERR(storage_path, SCK_SERVERERR_PARAM);
  nfi_sck_server_keepConnected(serv);
  NULL_RET_ERR(serv->private_info, SCK_SERVERERR_PARAM);
