...
void nfs_to_nfi_attr(struct nfi_attr *nfi_att, fattr *nfs_att)
{
	...
	/* u_long fsid;  */
	/* u_long fileid;*/
	nfi_att->at_atime = (time_t)nfs_att->atime.seconds + (1000 * nfs_att->atime.useconds);
	nfi_att->at_mtime = (time_t)nfs_att->mtime.seconds + (1000 * nfs_att->mtime.useconds);
	nfi_att->at_ctime = (time_t)nfs_att->ctime.seconds + (1000 * nfs_att->ctime.useconds);
...
int nfi_nfs_rename(struct nfi_server *serv, char *old_url, char *new_url)
{
	...
	/* obtain the directory file handle */
	ret = base_urlstr_parse_url(old_url, NULL, NULL, NULL, server, NULL, dir_src);
	if (ret < 0)
	{
		nfs_err(NFSERR_URL);
	...
	/* elimina del dir la parte del fichero */
	ret = base_path_misc_get_name_file(file_src, dir_src);
	if (ret < 0)
	{
		nfs_err(NFSERR_URL);
		fprintf(stderr, "nfi_nfs_rename: url %s incorrect.\n", old_url);
	...
	/* obtain the directory file handle */
	ret = base_urlstr_parse_url(new_url, NULL, NULL, NULL, server, NULL, dir_dest);
	if (ret < 0)
	{
		nfs_err(NFSERR_URL);
		fprintf(stderr, "nfi_nfs_rename: url %s incorrect.\n", new_url);
	...
	/* elimina del dir la parte del fichero */
	ret = base_path_misc_get_name_file(file_dest, dir_dest);
	if (ret < 0)
	{
		nfs_err(NFSERR_URL);
		fprintf(stderr, "nfi_nfs_rename: url %s incorrect.\n", new_url);
	...
	get_dir_with_url(serv->url, dir_src);
	get_dir_with_url(serv->url, dir_dest);
