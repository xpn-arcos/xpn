 ...
 */
  ...
  int tcp_server_write_operation ( int sd, struct st_tcp_server_msg *head )
  {
    ...
    debug_info("[NFI-MPI] (ID=%s) tcpClient_write_data: begin               HEAD_TYPE:%d\n", head->id, sizeof(head->type)) ;
  ...
  int nfi_tcp_server_destroy ( struct nfi_server *serv )
  {
    ...
    if (ret < 0) {
      debug_error("[NFI-MPI]: tcpClient_comm_destroy fails :-(") ;
    }
  ...
  int nfi_tcp_server_disconnect ( struct nfi_server *serv )
  {
    ...
    if (ret < 0) {
      debug_error("[NFI-MPI]: tcpClient_comm_disconnect fails :-(") ;
    }
  ...
  int nfi_tcp_server_open ( struct nfi_server *serv,  char *url, struct nfi_fhandle *fho )
  {
    ...
    // private_info...
    server_aux = (struct nfi_tcp_server_server *) serv->private_info;
    debug_info("[NFI-MPI] nfi_tcp_server_open(ID=%s): begin %s\n",server_aux->id,url) ;
  ...
  int nfi_tcp_server_create (struct nfi_server *serv,  char *url, struct nfi_attr *attr, struct nfi_fhandle  *fh)
  {
    ...
    // private_info...
    server_aux = (struct nfi_tcp_server_server *) serv->private_info;
    debug_info("[NFI-MPI] nfi_tcp_server_create(ID=%s): begin %s\n", server_aux->id, url) ;
  ...
  ssize_t nfi_tcp_server_read ( struct nfi_server *serv, struct nfi_fhandle *fh, void *buffer, off_t offset, size_t size )
  {
    ...
    // private_info
    server_aux = (struct nfi_tcp_server_server *) serv->private_info;
    debug_info("[NFI-MPI] nfi_tcp_server_read(%s): begin off %d size %d\n",server_aux->id,(int)offset, (int)size) ;
    ...
    else
    {
      ...
      do
      {
        ...
        debug_info("[NFI-MPI] nfi_tcp_server_read(ID=%s): (1)tcpClient_read_data = %d.\n",server_aux->id, ret) ;
        if (ret == -1){
        ...
        if (req.size > 0) {
          debug_info("[NFI-MPI] nfi_tcp_server_read(ID=%s): (2)tcpClient_read_data = %d. size = %d\n",server_aux->id, ret, req.size) ;
          ret = tcpClient_read_data(server_aux->params.server, (char *)buffer+cont, req.size, msg.id) ;
          debug_info("[NFI-MPI] nfi_tcp_server_read(ID=%s): (2)tcpClient_read_data = %d.\n",server_aux->id, ret) ;
          if (ret == -1) {
  ...
  ssize_t nfi_tcp_server_write ( struct nfi_server *serv, struct nfi_fhandle *fh, void *buffer, off_t offset, size_t size )
  {
    ...
    // private_info...
    server_aux = (struct nfi_tcp_server_server *) serv->private_info;
    debug_info("[NFI-MPI] nfi_tcp_server_write(ID=%s): begin off %d size %d\n",server_aux->id,(int)offset, (int)size) ;
    fh_aux     = (struct nfi_tcp_server_fhandle *) fh->priv_fh;
    ...
    else
    {
      ...
      do
      {
        if (diff > buffer_size)
        {
          ret = tcpClient_write_data(server_aux->params.server, (char *)buffer + cont, buffer_size, msg.id) ;
          if (ret == -1) {
      ...
      debug_info("[NFI-MPI] nfi_tcp_server_write(ID=%s): write %s off %d size %d (err:%d).\n",server_aux->id,fh->url,(int)offset,(int)size,(int)req.size) ;
      if (req.size < 0)
      {
  ...
  int nfi_tcp_server_close ( struct nfi_server *serv,  struct nfi_fhandle *fh )
  {
    ...
    // private_info...
    server_aux = (struct nfi_tcp_server_server *) serv->private_info;
    debug_info("[NFI-MPI] nfi_tcp_server_close(ID=%s): begin\n",server_aux->id) ;

    // without session -> just return ok
    if (server_aux->params.xpn_session == 0)
    {
      debug_info("[NFI-MPI] nfi_tcp_server_close(ID=%s): end\n", server_aux->id) ;
      return 1;
    }
    ...
    else
    {
      ...
      debug_info("[NFI-MPI] nfi_tcp_server_close(ID=%s): close -> %d \n",server_aux->id,msg.u_st_tcp_server_msg.op_close.fd) ;
    }
  ...
  int nfi_tcp_server_remove ( struct nfi_server *serv,  char *url )
  {
    ...
    // private_info...
    server_aux = (struct nfi_tcp_server_server *) serv->private_info;
    debug_info("[NFI-MPI] nfi_tcp_server_remove(%s): begin %s\n",server_aux->id, url) ;
    if (server_aux == NULL){
  ...
  int nfi_tcp_server_preload(struct nfi_server *serv, char *url, char *virtual_path, char *storage_path, int opt)
  {
    ...
    // get private_info...
    server_aux = (struct nfi_tcp_server_server *) serv->private_info;
    debug_info("[NFI-MPI] nfi_tcp_server_preload(ID=%s): begin %s - %s \n", server_aux->id, virtual_path, storage_path) ;
    ...
    server_aux = (struct nfi_tcp_server_server *) serv->private_info;
    debug_info("[NFI-MPI] nfi_tcp_server_preload(ID=%s): preload %s in server %s.\n",server_aux->id,virtual_path,serv->server) ;
    ...
    debug_info("[NFI-MPI] nfi_tcp_server_preload(ID=%s): end %s - %s = %d\n", server_aux->id,virtual_path, storage_path, ret) ;
    if (ret == -1) {
  ...
  int nfi_tcp_server_flush ( struct nfi_server *serv,  char *url, char *virtual_path, char *storage_path, int opt )
  {
    ...
    // private_info...
    server_aux = (struct nfi_tcp_server_server *) serv->private_info;
    debug_info("[NFI-MPI] nfi_tcp_server_flush(ID=%s): begin %s - %s \n", server_aux->id,virtual_path, storage_path) ;
    ...
    server_aux = (struct nfi_tcp_server_server *) serv->private_info;
    debug_info("[NFI-MPI] nfi_tcp_server_flush(ID=%s): open %s in server %s.\n",server_aux->id,virtual_path,serv->server) ;
