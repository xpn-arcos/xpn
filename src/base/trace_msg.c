...
/*
 *  Copyright 2020-2023 Felix Garcia Carballeira, Diego Camarmas Alonso, Alejandro Calderon Mateos, Luis Miguel Sanchez
...
int base_trace_prom_init(void) {
    ...
    bar_counter =
        prom_collector_registry_must_register_metric(prom_counter_new(counter_name, "counter for bar", 0, NULL));
 ...
 *    Nothing.
 */
void base_trace_msg_do_print(char *fto, ...) {
...
void base_trace_msg_variable_printf(int line, char *name, long pid, int type, char *fto, va_list vl) {
    ...
    base_trace_msg_do_print("trace(%i,\"%s\",%li,%i,\"%s\").", line, name, pid, type, msg);
    free(msg);
}
