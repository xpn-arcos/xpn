...
#ifdef DBG_NFI 
	printf("[NFI]ID=%s)mpiServer_write_data: begin\n", head->id);	
#endif
...
int nfi_mpiServer_init(char *url, struct nfi_server *serv, struct nfi_attr_server *attr)
{
...
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_init: begin\n");
#endif	

#ifdef DBG_NFI 
	printf("[NFI]nfi_mpiServer_init: sizeof(struct st_mpiServer_msg) = %d.\n", sizeof(struct st_mpiServer_msg));
#endif	
	...
	if(ret <0 ){
		...
		fprintf(stderr,"nfi_mpiServer_init: url %s incorrect.\n",url);
		free(serv->ops);
		return -1;
	}
...
#ifdef  DBG_NFI
	printf("[NFI]nfi_mpiServer_init: mpiServer_write_data\n");
#endif
	mpiServer_read_data(server_aux->sd, (char *)server_aux->id, MPISERVER_ID, msg.id);  
#ifdef  DBG_NFI
	printf("[NFI]nfi_mpiServer_init: mpiServer_read_data id = %s\n",server_aux->id);
#endif
	/******************************************/
...
#endif	
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_init(ID=%s): end\n",server_aux->id);
#endif
	return 0;
}
...
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_getattr(ID=%s): begin\n",server_aux->id);
#endif	
	if (attr == NULL){
		mpiServer_err(MPISERVERERR_PARAM);
		return -1;
...
#ifdef DBG_NFI 
	printf("[NFI]nfi_mpiServer_getattr(ID=%s): getattr: <- %d\n", server_aux->id, req.status);
#endif
	/*****************************************/	
...
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_getattr(ID=%s): end\n",server_aux->id);
#endif
	return req.status;
}
...
int nfi_mpiServer_preload(struct nfi_server *serv, char *url, char *virtual_path, char *storage_path, int opt)
{
	...
	strcpy(msg.id,server_aux->id);
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_preload(ID=%s): begin %s - %s \n", server_aux->id, virtual_path, storage_path);
#endif
	if(url == NULL){
		mpiServer_err(MPISERVERERR_PARAM);
		return -1;
	...
	server_aux = (struct nfi_mpiServer_server *) serv->private_info;
#ifdef DBG_NFI 
	printf("[NFI]nfi_mpiServer_preload(ID=%s): preload %s in server %s.\n",server_aux->id,virtual_path,serv->server);
#endif
...
int nfi_mpiServer_flush(struct nfi_server *serv,  char *url, char *virtual_path, char *storage_path, int opt)
{
	...
	strcpy(msg.id,server_aux->id);
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_flush(ID=%s): begin %s - %s \n", server_aux->id,virtual_path, storage_path);
#endif
	if(url == NULL){
		mpiServer_err(MPISERVERERR_PARAM);
		return -1;
	...
	server_aux = (struct nfi_mpiServer_server *) serv->private_info;
#ifdef DBG_NFI 
	printf("[NFI]nfi_mpiServer_flush(ID=%s): open %s in server %s.\n",server_aux->id,virtual_path,serv->server);
#endif
	...
	msg.u_st_mpiServer_msg.op_flush.opt = opt;
        ...
        mpiServer_write_operation(server_aux->sd, &msg);
#ifdef DBG_NFI 
	printf("[NFI]nfi_mpiServer_flush(ID=%s): flush: -> %s \n",server_aux->id,msg.u_st_mpiServer_msg.op_flush.virtual_path);
#endif
	mpiServer_read_data(server_aux->sd, (char *)&ret, sizeof(int), msg.id);
#ifdef DBG_NFI 
	printf("[NFI]nfi_mpiServer_flush(ID=%s): flush: <- %d \n",server_aux->id,ret);
#endif
	/*****************************************/
	
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_flush(ID=%s): end %s - %s = %d\n", server_aux->id,virtual_path, storage_path, ret);
#endif
	return 0;
}
...
int nfi_mpiServer_open(struct nfi_server *serv,  char *url, struct nfi_fhandle *fho )
{
	...
	strcpy(msg.id,server_aux->id);
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_open(ID=%s): begin %s\n",server_aux->id,url);
#endif
	if(url[strlen(url)-1] == '/'){
...
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_open(ID=%s): end\n",server_aux->id);
#endif
	return 0;
...
int nfi_mpiServer_close(struct nfi_server *server,  struct nfi_fhandle *fh){
	...
	strcpy(msg.id,server_aux->id);
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_close(ID=%s): begin\n",server_aux->id);
#endif
	if (server == NULL){
		mpiServer_err(MPISERVERERR_PARAM);
		return -1;
	...
	fh->server = NULL;
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_close(ID=%s): end\n",server_aux->id);
#endif
			...
			//off_t offset)
{
	...
	strcpy(msg.id,server_aux->id);
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_read(%s): begin off %d size %d\n",server_aux->id,(int)offset, (int)size);
#endif
	if (serv == NULL){
		mpiServer_err(MPISERVERERR_PARAM);
		return -1;
	...
	do{
...
#ifdef DBG_NFI 
		printf("[NFI]nfi_mpiServer_read(ID=%s): (1)mpiServer_read_data = %d.\n",server_aux->id, ret);
#endif		
		if(ret == -1){
		...
		if(req.size > 0){
#ifdef DBG_NFI 
		printf("[NFI]nfi_mpiServer_read(ID=%s): (2)mpiServer_read_data = %d. size = %d cont = %d\n",server_aux->id, ret, req.size, cont);
#endif		
			ret = mpiServer_read_data(server_aux->sd, (char *)buffer+cont, req.size, msg.id);
#ifdef DBG_NFI 
			printf("[NFI]nfi_mpiServer_read(ID=%s): (2)mpiServer_read_data = %d.\n",server_aux->id, ret);
#endif		
			if(ret == -1){
...
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_read(ID=%s): end\n",server_aux->id);
#endif
	return req.size;
}
			...
			//off_t offset)
{
	...
	strcpy(msg.id,server_aux->id);
#ifdef  DBG_NFI
	printf("[NFI]nfi_mpiServer_write(ID=%s): begin off %d size %d\n",server_aux->id,(int)offset, (int)size);
#endif
	if(size == 0){
		return 0;
	}	
	...
	/*****************************************/
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_write(ID=%s): write %s off %d size %d (err:%d).\n",server_aux->id,fh->url,(int)offset,(int)size,(int)req.size);
#endif
	if(req.size < 0){
...
#ifdef  DBG_NFI
	printf("[NFI]nfi_mpiServer_write(ID=%s): end\n",server_aux->id);
#endif
	return req.size;
}
...
int nfi_mpiServer_create(struct nfi_server *serv,  char *url, struct nfi_attr *attr, struct nfi_fhandle  *fh){
	...
	strcpy(msg.id,server_aux->id);
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_create(ID=%s): begin %s\n",server_aux->id,url);
#endif
	if (serv == NULL){
		mpiServer_err(MPISERVERERR_PARAM);
		return -1;
...
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_create(ID=%s): end\n",server_aux->id);
#endif
	return 0;
}
...
int nfi_mpiServer_remove(struct nfi_server *serv,  char *url)
{
	...
	strcpy(msg.id,server_aux->id);
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_remove(%s): begin %s\n",server_aux->id, url);
#endif	
	if (serv == NULL){
		mpiServer_err(MPISERVERERR_PARAM);
		return -1;
	...
	/*****************************************/
#ifdef  DBG_NFI 
	printf("[NFI]nfi_mpiServer_remove(ID=%s): end \n",server_aux->id);
#endif
	return 0;
}
