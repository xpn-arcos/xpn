 ...
 */
   ...
   /* ... Functions / Funciones ......................................... */
	  ...
	  // Lookup port name
          ...
          if (MPI_SUCCESS != ret) {
              debug_error("Server[%d]: MPI_Lookup_name fails :-(", server_aux->rank) ;
              free(serv->ops);
              free(server_aux);
              return -1 ;
          }
          ...
          if (MPI_SUCCESS != ret) {
              debug_error("Client[%d]: MPI_Comm_connect fails :-(", server_aux->rank) ;
              free(serv->ops);
              free(server_aux);
              return -1 ;
          }
...
//	nfi_worker_init(serv->wrk, serv, 0);
	  ...
	  msg.type = -1;
          ...
          if (MPI_SUCCESS != ret) {
              debug_error("Client[%d]: MPI_Comm_disconnect fails :-(", server_aux->rank) ;
          }
