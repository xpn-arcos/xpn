...
int mq_server_comm_init(mq_server_param_st * params) 
{
    ...
    ret = ns_publish(params -> dns_file, "mq_server", params -> name, ip, params -> port);
    if (ret < 0) {
        fprintf(stderr, "ns_publish(dns_file:%s, protocol:mq_server, name:%s, ip:%s, port:%s) -> %d\n", params -> dns_file, params -> name, ip, params -> port, ret);
        return -1;
    }
...
int mq_server_comm_destroy(mq_server_param_st * params) {
    ...
    for (int i = 0; i < params -> size; ++i) {
        if (params -> rank == i) {
            ret = ns_unpublish(params -> dns_file, "mq_server", params -> srv_name);
            if (ret < 0) {
...
int mq_server_comm_connect(mq_server_param_st * params, char * server_name, int port_number) {
    ...
    if (NULL == hp) {
        ...
        fprintf(stderr, "mq_server_comm_connect: error gethostbyname (%s,%d)\n", server_name, port_number);
        return -1;
    }
    ...
    if (ret < 0) {
        //mq_server_err(MQ_SERVERERR_MEMORY);
        fprintf(stderr, "nfi_mq_server_init: error in connect (%s,%d)\n", server_name, port_number);
        perror("nfi_mq_server_init:");
        return -1;
    }
...
ssize_t mq_server_comm_read_operation(mq_server_param_st * params, int fd, char * data, ssize_t size, int * rank_client_id) {
    ...
    ret = mq_server_comm_read_data(params, fd, data, size * sizeof(int), * rank_client_id); //      Nuevo
    if (ret < 0) {
        debug_warning("[SRV_TCP_COMM] server: mq_server_comm_read_op fails : %d\n", ret);
        return ret;
    }
...
ssize_t mq_server_comm_write_data(mq_server_param_st * params, int fd, char * data, ssize_t size, __attribute__((__unused__)) int rank_client_id) //TODO rank client
{
    int ret, cont;
...
ssize_t mq_server_comm_read_data(mq_server_param_st * params, int fd, char * data, ssize_t size, __attribute__((__unused__)) int rank_client_id) //TODO rank client
{
    int ret, cont;
