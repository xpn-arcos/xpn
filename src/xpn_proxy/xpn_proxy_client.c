...
ssize_t write_n_bytes(int sock,
 ...
 * @return: Socket descriptor on success, -1 on error.
 */
int d_send_receive_rn(struct st_xpn_server_msg * pr, struct st_xpn_server_status * res,
    const char * old_path,
        const char * new_path, int old_path_len, int new_path_len) {
    int ret, sd_server;
 ...
 * @return: 0 on success, -1 on error.
 */
int xpn_rename(const char * path,
    const char * newpath) {
    struct st_xpn_server_msg pr;
    struct st_xpn_server_status res;
    int path_len, path_len_max;
