  ...
  void worker_pool_function ( void )
  {
    ...
    {
        // Dequeue operation
        th = mpi_server_worker_pool_dequeue ( );
        th.function( th );
    }
  ...
  int mpi_server_worker_pool_init ( void )
  {
    ...
    {
      debug_info("[WORKERS] pthread_create: create_thread mpi_server_worker_pool_init\n") ;
  ...
  void mpi_server_worker_pool_enqueue ( mpi_server_param_st *params, MPI_Comm sd, int op_type, int rank_client_id, void (*worker_function)(struct st_th))
  {
    ...
    debug_info("[WORKERS] client(%d): mpi_server_worker_pool_enqueue(...) lock\n", rank_client_id);
    pthread_mutex_lock(&m_pool);
    while (n_operation == MAX_OPERATIONS){
      debug_info("[WORKERS] client(%d): mpi_server_worker_pool_enqueue(...) wait c_pool_no_full\n", rank_client_id);
      pthread_cond_wait(&c_pool_no_full, &m_pool);
    }

    debug_info("[WORKERS] client(%d): mpi_server_worker_pool_enqueue(...) copy arguments\n", rank_client_id);
    ...
    debug_info("[WORKERS] client(%d): mpi_server_worker_pool_enqueue(...) enqueue\n", rank_client_id);
    operations_buffer[enq_pos] = st_worker;
    enq_pos = (enq_pos+1) % MAX_OPERATIONS;
    n_operation++;

    debug_info("[WORKERS] client(%d): mpi_server_worker_pool_enqueue(...) signal c_poll_no_empty\n", rank_client_id);
    pthread_cond_signal(&c_poll_no_empty);
    debug_info("[WORKERS] client(%d): mpi_server_worker_pool_enqueue(...) unlock\n", rank_client_id);
  ...
  struct st_th mpi_server_worker_pool_dequeue ( void )
  {
    ...
    debug_info("[WORKERS] client(%d): mpi_server_worker_pool_dequeue(...) lock\n", th.id);
    pthread_mutex_lock(&m_pool);

    while (n_operation == 0) {
      debug_info("[WORKERS] client(%d): mpi_server_worker_pool_dequeue(...) wait c_poll_no_empty\n", th.id);
      pthread_cond_wait(&c_poll_no_empty, &m_pool);
    }

    debug_info("[WORKERS] thread id = %ld\n", pthread_self());

    debug_info("[WORKERS] client(%d): mpi_server_worker_pool_dequeue(...) dequeue\n", th.id);
    th = operations_buffer[deq_pos];
    deq_pos = (deq_pos + 1) % MAX_OPERATIONS;
    n_operation--;

    if ( pool_end == 1 || th.type_op == MPI_SERVER_FINALIZE ) {
      debug_info("[WORKERS] client(%d): mpi_server_worker_pool_dequeue(...) unlock end\n", th.id);
      pthread_mutex_unlock(&m_pool);
      debug_info("[WORKERS] client(%d): mpi_server_worker_pool_dequeue(...) exit\n", th.id);
      pthread_exit(0);
    }

    debug_info("[WORKERS] client(%d): mpi_server_worker_pool_dequeue(...) signal c_pool_no_full\n", th.id);
    pthread_cond_signal(&c_pool_no_full);

    debug_info("[WORKERS] client(%d): mpi_server_worker_pool_dequeue(...) unlock\n", th.id);
    pthread_mutex_unlock(&m_pool);
  ...
  void mpi_server_worker_pool_destroy ( void )
  {
    ...
    // update pool_end...
    debug_info("[WORKERS] client: mpi_server_worker_pool_destroy(...) lock\n");
    pthread_mutex_lock(&m_pool_end);
    pool_end=1;
    debug_info("[WORKERS] : mpi_server_worker_pool_destroy(...) unlock\n");
    pthread_mutex_unlock(&m_pool_end);

    for (int i = 0; i < POOL_MAX_THREADS; ++i)
    {
      mpi_server_worker_pool_enqueue ( NULL, (MPI_Comm)0, MPI_SERVER_FINALIZE, 0, NULL );
    }

    debug_info("[WORKERS] : mpi_server_worker_pool_destroy(...) lock\n");
    pthread_mutex_lock(&m_pool);
    debug_info("[WORKERS] : mpi_server_worker_pool_destroy(...) broadcast\n");
    ...
    for (int i=0; i<POOL_MAX_THREADS; i++)
    {
      debug_info("[WORKERS] : mpi_server_worker_pool_destroy(...) join\n");
    ...
    debug_info("[WORKERS] : mpi_server_worker_pool_destroy(...) destroy\n");
