 ...
 *    Nothing.
 */
void mpi_server_op_open_ws(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                           int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_open_wos(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                            int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_creat_ws(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                            int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_creat_wos(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                             int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_read_ws(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                           int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_read_wos(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                            int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_write_ws(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                            int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_write_wos(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                             int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_close_ws(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                            int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_rm_async(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                            int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_setattr(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                           int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_getattr(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                           int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_opendir(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                           int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_readdir(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                           int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_closedir(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                            int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_rmdir_async(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                               int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_preload(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                           int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_getnodename(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                               int rank_client_id);
 ...
 *    Nothing.
 */
void mpi_server_op_fstat(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                         int rank_client_id);  // TODO: implement
 ...
 *    Nothing.
 */
void mpi_server_op_getid(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                         int rank_client_id);  // TODO: call in switch
...
int mpi_server_do_operation(struct st_th *th, int *the_end) {
    ...
    switch (th->type_op) {
        // File API
        case MPI_SERVER_OPEN_FILE_WS:
            ret = mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_open),
                                            sizeof(struct st_mpi_server_open), th->rank_client_id);
            if (ret != -1) {
        ...
        case MPI_SERVER_OPEN_FILE_WOS:
            ...
            break;
        case MPI_SERVER_CREAT_FILE_WS:
            ret = mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_creat),
                                            sizeof(struct st_mpi_server_creat), th->rank_client_id);
            if (ret != -1) {
        ...
        case MPI_SERVER_CREAT_FILE_WOS:
            ...
            break;
        case MPI_SERVER_READ_FILE_WS:
            ret = mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_read),
                                            sizeof(struct st_mpi_server_read), th->rank_client_id);
            if (ret != -1) {
        ...
        case MPI_SERVER_READ_FILE_WOS:
            ...
            break;
        case MPI_SERVER_WRITE_FILE_WS:
            ret = mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_write),
                                            sizeof(struct st_mpi_server_write), th->rank_client_id);
            if (ret != -1) {
        ...
        case MPI_SERVER_WRITE_FILE_WOS:
            ...
            break;
        case MPI_SERVER_CLOSE_FILE_WS:
            ret = mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_close),
                                            sizeof(struct st_mpi_server_close), th->rank_client_id);
            if (ret != -1) {
        ...
        // Metadata API
        case MPI_SERVER_RM_FILE:
            ret = mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_rm),
                                            sizeof(struct st_mpi_server_rm), th->rank_client_id);
            if (ret != -1) {
        ...
        case MPI_SERVER_RM_FILE_ASYNC:
            ...
            break;
        case MPI_SERVER_RENAME_FILE:
            ret = mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_rename),
                                            sizeof(struct st_mpi_server_rename), th->rank_client_id);
            if (ret != -1) {
        ...
        case MPI_SERVER_GETATTR_FILE:
            ret =
                mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_getattr),
                                          sizeof(struct st_mpi_server_getattr), th->rank_client_id);
            if (ret != -1) {
        ...
        case MPI_SERVER_SETATTR_FILE:
            ret =
                mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_setattr),
                                          sizeof(struct st_mpi_server_setattr), th->rank_client_id);
            if (ret != -1) {
        ...
        // Directory API
        case MPI_SERVER_MKDIR_DIR:
            ret = mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_mkdir),
                                            sizeof(struct st_mpi_server_mkdir), th->rank_client_id);
            if (ret != -1) {
        ...
        case MPI_SERVER_OPENDIR_DIR:
            ret =
                mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_opendir),
                                          sizeof(struct st_mpi_server_opendir), th->rank_client_id);
            if (ret != -1) {
        ...
        case MPI_SERVER_READDIR_DIR:
            ret =
                mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_readdir),
                                          sizeof(struct st_mpi_server_readdir), th->rank_client_id);
            if (ret != -1) {
        ...
        case MPI_SERVER_CLOSEDIR_DIR:
            ret =
                mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_closedir),
                                          sizeof(struct st_mpi_server_closedir), th->rank_client_id);
            if (ret != -1) {
                mpi_server_op_closedir(th->params, (MPI_Comm)th->sd, &head, th->rank_client_id);
            }
            break;
        case MPI_SERVER_RMDIR_DIR:
            ret = mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_rmdir),
                                            sizeof(struct st_mpi_server_rmdir), th->rank_client_id);
            if (ret != -1) {
        ...
        case MPI_SERVER_PRELOAD_FILE:
            ret =
                mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_preload),
                                          sizeof(struct st_mpi_server_preload), th->rank_client_id);
            if (ret != -1) {
                mpi_server_op_preload(th->params, (MPI_Comm)th->sd, &head, th->rank_client_id);
            }
            break;
        case MPI_SERVER_FLUSH_FILE:
            ret = mpi_server_comm_read_data(th->params, (MPI_Comm)th->sd, (char *)&(head.u_st_mpi_server_msg.op_flush),
                                            sizeof(struct st_mpi_server_flush), th->rank_client_id);
            if (ret != -1) {
...
void mpi_server_op_creat_ws(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
    ...
    // show debug info
    debug_info("[MPI-SERVER-OPS] (ID=%s) CREAT(%s)=%d\n", params->srv_name, head->u_st_mpi_server_msg.op_creat.path,
               fd);
}
...
void mpi_server_op_read_ws(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
    ...
    debug_info("[MPI-SERVER-OPS] (ID=%s) begin read: fd %d offset %d size %d ID=x\n", params->srv_name,
               head->u_st_mpi_server_msg.op_read.fd, (int)head->u_st_mpi_server_msg.op_read.offset,
    ...
    do {
        ...
        // lseek and read data...
        filesystem_lseek(head->u_st_mpi_server_msg.op_read.fd, head->u_st_mpi_server_msg.op_read.offset + cont,
                         SEEK_SET);  // TODO: check error
        ...
        if (req.size < 0) {
            ...
            mpi_server_comm_write_data(params, sd, (char *)&req, sizeof(struct st_mpi_server_write_req),
                                       rank_client_id);
    ...
    debug_info("[MPI-SERVER-OPS] (ID=%s) end READ: fd %d offset %d size %d ID=x\n", params->srv_name,
               head->u_st_mpi_server_msg.op_read.fd, (int)head->u_st_mpi_server_msg.op_read.offset, size);
}
...
void mpi_server_op_read_wos(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
    ...
    debug_info("[MPI-SERVER-OPS] (ID=%s) begin read: path %s offset %d size %d ID=x\n", params->srv_name,
               head->u_st_mpi_server_msg.op_read.path, (int)head->u_st_mpi_server_msg.op_read.offset,
               head->u_st_mpi_server_msg.op_read.size);
    ...
    debug_info("[MPI-SERVER-OPS] (ID=%s) end READ: path %s offset %d size %d ID=x\n", params->srv_name,
               head->u_st_mpi_server_msg.op_read.path, (int)head->u_st_mpi_server_msg.op_read.offset, size);
}
...
void mpi_server_op_write_ws(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
    ...
    debug_info("[MPI-SERVER-OPS] (ID=%s) begin write: fd %d ID=x\n", params->srv_name,
               head->u_st_mpi_server_msg.op_write.fd);
    ...
    do {
        ...
        off_t ret = filesystem_lseek(head->u_st_mpi_server_msg.op_write.fd,
                                     head->u_st_mpi_server_msg.op_write.offset + cont, SEEK_SET);
        // TODO: check error
        if ((off_t)-1 == ret) {
            req.size = -1;
            break;
...
void mpi_server_op_write_wos(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
    ...
    debug_info("[MPI-SERVER-OPS] (ID=%s) begin write: path %s ID=x\n", params->srv_name,
               head->u_st_mpi_server_msg.op_write.path);
...
void mpi_server_op_rm_async(mpi_server_param_st *params, __attribute__((__unused__)) MPI_Comm sd,
                            struct st_mpi_server_msg *head, __attribute__((__unused__)) int rank_client_id) {
    char path[PATH_MAX];
...
void mpi_server_op_rename(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
                          int rank_client_id) {
    ...
    // show debug info
    debug_info("[MPI-SERVER-OPS] (ID=%s) RENAME(old_path=%s; new_path)\n", head->u_st_mpi_server_msg.op_rename.old_url,
               head->u_st_mpi_server_msg.op_rename.new_url);
}
...
void mpi_server_op_setattr(mpi_server_param_st *params, __attribute__((__unused__)) MPI_Comm sd,
                           struct st_mpi_server_msg *head, __attribute__((__unused__)) int rank_client_id) {
    // check params...
    if (NULL == params) {
        return;
...
void mpi_server_op_rmdir_async(__attribute__((__unused__)) mpi_server_param_st *params,
                               __attribute__((__unused__)) MPI_Comm sd, struct st_mpi_server_msg *head,
...
void mpi_server_op_preload(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head,
    ...
    ret = base_urlstr_parse_url(head->u_st_mpi_server_msg.op_preload.virtual_path, protocol, user, pass, machine, port,
                                file);
    ...
    debug_info("[MPI-SERVER-OPS] (ID=%s) PRELOAD(%s,%s) -> %d\n", params->srv_name,
               head->u_st_mpi_server_msg.op_preload.virtual_path, head->u_st_mpi_server_msg.op_preload.storage_path,
               ret);
    return;
}
...
void mpi_server_op_flush(mpi_server_param_st *params, MPI_Comm sd, struct st_mpi_server_msg *head, int rank_client_id) {
    ...
    ret = base_urlstr_parse_url(head->u_st_mpi_server_msg.op_flush.virtual_path, protocol, user, pass, machine, port,
                                file);
    ...
    debug_info("[MPI-SERVER-OPS] (ID=%s) FLUSH(%s)\n", params->srv_name,
               head->u_st_mpi_server_msg.op_flush.virtual_path);
}

/************************************************
 *  ... Functions: FS Metadata API
 ***********************************************/
void mpi_server_op_getnodename(mpi_server_param_st *params, MPI_Comm sd,
                               __attribute__((__unused__)) struct st_mpi_server_msg *head, int rank_client_id) {
    ...
    mpi_server_comm_write_data(params, sd, (char *)serv_name, HOST_NAME_MAX,
                               rank_client_id);  // Send one single message
    mpi_server_comm_write_data(params, sd, (char *)params->dirbase, PATH_MAX,
                               rank_client_id);  // Send one single message
    // mpi_server_comm_write_data(params, sd, (char *)params->sem_name_server, PATH_MAX, rank_client_id); // Send one
    // single message
