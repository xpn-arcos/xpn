...
int main ( int argc, char *argv[] )
{
	...
	{
	    printf("\n") ;
	    printf(" Usage: %s <full path> <megabytes to write-read> <buffer size> <stop/kill/mark> <num of servers with error> \n", argv[0]) ;
	    printf("\n") ;
	    printf(" Example:") ;
	...
	if (!((strcmp(argv[4],"stop") == 0)||(strcmp(argv[4],"kill") == 0)||(strcmp(argv[4],"mark") == 0)))
	{
	...
	printf("MB to test: %f Buffer size: %f bytes %d\n", mb_file, buff_size_mb, file_size);
	...
	if ((strcmp(argv[4],"stop") == 0) || (strcmp(argv[4],"kill") == 0)){
		// xpn-destroy
		ret = xpn_destroy();
	...
	if (strcmp(argv[4],"stop") == 0){
		...
		printf("stop/kill parameter: %s is not stop, kill or mark",argv[4]);
		freeBuffers();
		return -1;
	}
