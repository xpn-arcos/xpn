...
)
{
    ...
    dir=TAKE_SAMPLE_DIR;
    sprintf (fname, "%s/%s.%d.%d.%d", dir, "ION", ncid, cid, lbuf); 
...
int main(int argc, char **argv)
{
    ...
    if(cid == 0 ){
    	system("/export/home/pato11-1/proyectos/xpn/expand-2.0/test/pruebas/sync.sh");
	printf("sync.sh\n");
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if(cid == 0 ){
        system("/export/home/pato11-1/proyectos/xpn/expand-2.0/test/pruebas/delete.sh");
        printf("delete.sh\n");
    }
    MPI_Barrier(MPI_COMM_WORLD);
