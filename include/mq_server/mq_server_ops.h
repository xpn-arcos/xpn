...
#define MQ_CHECK_MAGIC_NUMBER(mdata)\
 ...
 #define MQ_METADATA_MAX_RECONSTURCTIONS 40
 /*
  *  Message struct
  */
 ...
 struct xpn_metadata {
     ...
     int data_nserv[MQ_METADATA_MAX_RECONSTURCTIONS]; // Array of number of servers to reconstruct
     int offsets[MQ_METADATA_MAX_RECONSTURCTIONS]; // Array indicating the block where new server configuration starts
 };
 ...
 struct st_mq_server_opendir { //NEW
     char path[PATH_MAX];
 };
 
 struct st_mq_server_readdir { //NEW
     DIR * dir;
 };
 
 struct st_mq_server_direntry { //NEW
     int end; //If end = 1 exist entry; 0 not exist
     struct dirent ret;
 };
 
 struct st_mq_server_closedir { //NEW
     DIR * dir;
 };
