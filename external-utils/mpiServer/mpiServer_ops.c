...
int mpiServer_create_spacename(char *path)
{
	...
	debug_printf("[OPS]%s)mpiServer_create_spacename: %s\n", MPISERVER_ALIAS_NAME_STRING, path);
...
int mpiServer_read_operation(int sd, struct st_mpiServer_msg *head){
	...
	case MPISERVER_CREAT_FILE:
		debug_printf("[OPS]%s)CREAT operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	...
	case MPISERVER_READ_FILE:
		debug_printf("[OPS]%s)READ operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	...
	case MPISERVER_WRITE_FILE:
		debug_printf("[OPS]%s)WRITE operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	...
	case MPISERVER_CLOSE_FILE:
		debug_printf("[OPS]%s)CLOSE operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	...
	case MPISERVER_RM_FILE:
		debug_printf("[OPS]%s)RM operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	...
	case MPISERVER_GETATTR_FILE:
		debug_printf("[OPS]%s)GETATTR operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	...
	case MPISERVER_SETATTR_FILE:
		debug_printf("[OPS]%s)SETATTR operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	...
	case MPISERVER_MKDIR_DIR:
		debug_printf("[OPS]%s)MKDIR operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	...
	case MPISERVER_RMDIR_DIR:
		debug_printf("[OPS]%s)RMDIR operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	...
	case MPISERVER_FLUSH_FILE:
		debug_printf("[OPS]%s)FLUSH operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	...
	case MPISERVER_PRELOAD_FILE:
		debug_printf("[OPS]%s)PRELOAD operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
        ...
        case MPISERVER_GETID:
                debug_printf("[OPS]%s)GETID operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
/*
...
*/	
	...
	case MPISERVER_FINALIZE:
		debug_printf("[OPS]%s)FINALIZE operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
/*
...
*/	
		break;
	case MPISERVER_END:
		debug_printf("[OPS]%s)END operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
/*
		ret = mpiServer_comm_readdata(sd, (char *)&head->u_st_mpiServer_msg.op_end, sizeof(struct st_mpiServer_end), head->id);
		if(ret == -1){
			return -1;
...
void mpiServer_op_open(int sd, struct st_mpiServer_msg *head){
	...
	debug_printf("[OPS]%s> begin open(%s) ID=%s -> %d\n",MPISERVER_ALIAS_NAME_STRING,s,head->id,fd);
	
	fd = open(s, O_RDWR);
	//if(fd == -1){
	//	mpiServer_create_spacename(s);
	//}	
	debug_printf("[OPS]%s> end open(%s) ID=%s ->%d\n",MPISERVER_ALIAS_NAME_STRING,s, head->id, fd);	
...
void mpiServer_op_creat(int sd, struct st_mpiServer_msg *head){
	...
	debug_printf("[OPS]%s)begin creat(%s) ID=%s ->\n",MPISERVER_ALIAS_NAME_STRING,s,head->id);
	...
	debug_printf("[OPS]%s)end creat(%s) ID=%s -> %d\n",MPISERVER_ALIAS_NAME_STRING,s,head->id,fd);
	mpiServer_comm_writedata(sd,(char *)&fd,sizeof(int), head->id);
	debug_printf("[OPS]%s)end CREAT operation from ID=%s ->%d\n",MPISERVER_ALIAS_NAME_STRING,head->id,fd);
}
...
void mpiServer_op_preload(int sd, struct st_mpiServer_msg *head){
	...
	debug_printf("[OPS]%s)begin preload(%s,%s) ID=%s\n", MPISERVER_ALIAS_NAME_STRING,
	...
	debug_printf("[OPS]%s)end preload(%s,%s) ID=%s -> %d\n",MPISERVER_ALIAS_NAME_STRING,
							head->u_st_mpiServer_msg.op_preload.virtual_path,
							head->u_st_mpiServer_msg.op_preload.storage_path,
							head->id, ret);
	...
	debug_printf("[OPS]%s)end PRELOAD operation from ID=%s\n", MPISERVER_ALIAS_NAME_STRING,head->id);
}
...
{
	debug_printf("[OPS]%s)begin close: fd %d ID=%s\n",MPISERVER_ALIAS_NAME_STRING,
	...
	debug_printf("[OPS]%s)end CLOSE operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
}
...
void mpiServer_op_rm(int sd, struct st_mpiServer_msg *head)
{
	...
	debug_printf("[OPS]%s)begin unlink: path %s ID=%s\n",MPISERVER_ALIAS_NAME_STRING,
						head->u_st_mpiServer_msg.op_rm.path,
						head->id);
	unlink(s);

	debug_printf("[OPS]%s)end unlink: path %s ID=%s\n",MPISERVER_ALIAS_NAME_STRING,
						head->u_st_mpiServer_msg.op_rm.path,
						head->id);
	debug_printf("[OPS]%s)end RM operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
}
...
void mpiServer_op_read(int sd, struct st_mpiServer_msg *head)
{
	...
	debug_printf("[OPS]%s)op_read: static buffer (%d) ID=%s\n",MPISERVER_ALIAS_NAME_STRING,MAX_BUFFER_SIZE,head->id);
#else
	char *buffer;
	SIZE = 0;
	debug_printf("[OPS]%s)op_read: variable buffer (%d) ID=%s\n",MPISERVER_ALIAS_NAME_STRING,MAX_BUFFER_SIZE,head->id);
#endif
	

	//
	
	debug_printf("[OPS]%s)begin read: fd %d offset %d size %d ID=%s\n",MPISERVER_ALIAS_NAME_STRING,
...
*/
	...
	//debug_printf("[OPS]%s)op_read: malloc(%d) ID=%s\n",MPISERVER_ALIAS_NAME_STRING,MAX_BUFFER_SIZE,head->id);
	...
	do{	
		...
		lseek64(head->u_st_mpiServer_msg.op_read.fd,
				...
				0);
#else
	        debug_printf("[OPS]%s)lseek: fd %d offset %d size %d ID=%s\n",MPISERVER_ALIAS_NAME_STRING,
						head->u_st_mpiServer_msg.op_read.fd,
	...
	//t2 = MPI_Wtime();
        ...
        debug_printf("[OPS]%s)end read: fd %d offset %d size %d ID=%s\n",MPISERVER_ALIAS_NAME_STRING,
                                                head->u_st_mpiServer_msg.op_read.fd,
                                                (int)head->u_st_mpiServer_msg.op_read.offset,
                                                size,
                                                head->id);
	
	//debug_printf("[OPS]%s)op_read: %f sec.\n",MPISERVER_ALIAS_NAME_STRING,t2 - t1);
	debug_printf("[OPS]%s)op_read: new_size %d\n",MPISERVER_ALIAS_NAME_STRING,size);
#ifdef _MALLOC_	
	debug_printf("[OPS]%s)op_read: free ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	free(buffer);
#endif
        debug_printf("[OPS]%s)end READ: fd %d offset %d size %d ID=%s\n",MPISERVER_ALIAS_NAME_STRING,
                                                head->u_st_mpiServer_msg.op_read.fd,
...
void mpiServer_op_write(int sd, struct st_mpiServer_msg *head){
	...
	debug_printf("[OPS]%s)begin write: fd %d ID=%sn",MPISERVER_ALIAS_NAME_STRING,
						head->u_st_mpiServer_msg.op_write.fd,
						head->id);	

/*
	debug_printf("[OPS]%s)op_write: fd %d\n",MPISERVER_ALIAS_NAME_STRING,head->u_st_mpiServer_msg.op_write.fd);
#ifdef _LARGEFILE64_
...
*/	
	...
	debug_printf("[OPS]%s)op_write: %d\n",MPISERVER_ALIAS_NAME_STRING, req.size);
	debug_printf("[OPS]%s)end write: fd %d ID=%sn",MPISERVER_ALIAS_NAME_STRING,
						head->u_st_mpiServer_msg.op_write.fd,
						head->id);	
	debug_printf("[OPS]%s)end WRITE operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
}
...
void mpiServer_op_mkdir(int sd, struct st_mpiServer_msg *head){
	...
	ret = mkdir(s, 0777);
	mpiServer_comm_writedata(sd,(char *)&ret,sizeof(int), head->id);
	debug_printf("[OPS]%s)end MKDIR operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
}
...
void mpiServer_op_rmdir(int sd, struct st_mpiServer_msg *head){
	...
	ret = rmdir(s);
	mpiServer_comm_writedata(sd,(char *)&ret,sizeof(int), head->id);
	debug_printf("[OPS]%s)end RMDIR operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
}

void mpiServer_op_setattr(int sd, struct st_mpiServer_msg *head){
	debug_printf("[OPS]%s)begin SETATTR operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
	debug_printf("[OPS]%s)SETATTR operation to be implemented !!\n",MPISERVER_ALIAS_NAME_STRING);
	debug_printf("[OPS]%s)end   SETATTR operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
}
...
void mpiServer_op_getattr(int sd, struct st_mpiServer_msg *head){
	...
	debug_printf("[OPS]%s)begin getattr(%s) ID=%sn",MPISERVER_ALIAS_NAME_STRING,
	...
	debug_printf("[OPS]%s)end getattr(%s) ID=%sn",MPISERVER_ALIAS_NAME_STRING,
						head->u_st_mpiServer_msg.op_getattr.path,	
						head->id);	
	debug_printf("[OPS]%s)end GETATTR operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
}



void mpiServer_op_getid(int sd, struct st_mpiServer_msg *head){
        debug_printf("[OPS]%s)begin GETID ID=%s\n",MPISERVER_ALIAS_NAME_STRING,
                                                head->id);
        //sprintf(s,"%s/", head->u_st_mpiServer_msg.op_rmdir.path);
        mpiServer_comm_writedata(sd,(char *)head->id, MPISERVER_ID, head->id);
        debug_printf("[OPS]%s)end GETID operation from ID=%s\n",MPISERVER_ALIAS_NAME_STRING,head->id);
}
