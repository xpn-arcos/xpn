...
int mpiServer_create_spacename(char *path){
	...
	int i;
#ifdef DBG_OPS	
	printf("[OPS]%s)mpiServer_create_spacename: %s\n", MYSERVER_ALIAS_NAME_STRING, path);
#endif
...
#ifdef DBG_OPS	
	printf("[OPS]%s)antes de read_operation: sizeof(struct st_mpiServer_msg) = %d.\n ", MYSERVER_ALIAS_NAME_STRING, sizeof(struct st_mpiServer_msg));
#endif
...
#endif
/*
		ret = mpiServer_comm_readdata(sd, (char *)&head->id, sizeof(MYSERVER_ID), head->id);
		if(ret == -1){
			return -1;
		}
*/	
...
#ifdef DBG_OPS	
	printf("[OPS]%s)begin flush(%s) ID=%s ->\n",MYSERVER_ALIAS_NAME_STRING, head->u_st_mpiServer_msg.op_flush.virtual_path, head->id);
#endif	
/*	
	sprintf(s_exe,"%s %s %s\n", MYSERVER_FLUSH_EXE, 
...
/*
#ifdef _LARGEFILE64_ 
	printf("[OPS]%s)op_read: offset %lld\n",MYSERVER_ALIAS_NAME_STRING,head->u_st_mpiServer_msg.op_read.offset);
#else
	printf("[OPS]%s)op_read: offset %d\n",MYSERVER_ALIAS_NAME_STRING,(int)head->u_st_mpiServer_msg.op_read.offset);
#endif
	printf("[OPS]%s)op_read: size %d\n",MYSERVER_ALIAS_NAME_STRING,head->u_st_mpiServer_msg.op_read.size);
#endif		
*/
#ifdef _MALLOC_
...
/*
#ifdef DBG_OPS	
	printf("[OPS]%s)op_write: fd %d\n",MYSERVER_ALIAS_NAME_STRING,head->u_st_mpiServer_msg.op_write.fd);
#ifdef _LARGEFILE64_ 
	printf("[OPS]%s)op_write: offset %lld\n",MYSERVER_ALIAS_NAME_STRING,head->u_st_mpiServer_msg.op_write.offset);
#else
	printf("[OPS]%s)op_write: offset %d\n",MYSERVER_ALIAS_NAME_STRING,(int)head->u_st_mpiServer_msg.op_write.offset);
#endif
	printf("[OPS]%s)op_write: size %d\n",MYSERVER_ALIAS_NAME_STRING,head->u_st_mpiServer_msg.op_write.size);
#endif
*/	
	//t1 = MPI_Wtime();
...
#endif
        //sprintf(s,"%s/", head->u_st_mpiServer_msg.op_rmdir.path);
        mpiServer_comm_writedata(sd,(char *)head->id, MYSERVER_ID, head->id);
#ifdef DBG_OPS    
