...
#ifdef DBG_OPS	
	printf("[OPS]%s)begin write: fd %d ID=%sn",MPISERVER_ALIAS_NAME_STRING, 
...
*/	
	...
	#endif		
#ifdef DBG_OPS	
	printf("[OPS]%s)end write: fd %d ID=%sn",MPISERVER_ALIAS_NAME_STRING, 
						head->u_st_mpiServer_msg.op_write.fd,
						head->id);	
#endif
#ifdef DBG_OPS	
...
#ifdef DBG_OPS	
	printf("[OPS]%s)begin getattr(%s) ID=%sn",MPISERVER_ALIAS_NAME_STRING, 
...
#ifdef DBG_OPS	
	printf("[OPS]%s)end getattr(%s) ID=%sn",MPISERVER_ALIAS_NAME_STRING, 
						head->u_st_mpiServer_msg.op_getattr.path,	
						head->id);	
#endif
#ifdef DBG_OPS	
