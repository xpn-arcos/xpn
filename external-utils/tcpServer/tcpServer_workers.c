 ...
 */
    ...
    */
    ...
    void tcpServer_destroy_worker_pool ( void ) {
        ...
        pool_end=1;
        debug_info("[WORKERS] client: mpiServer_destroy_worker_pool(...) unlock\n");
        pthread_mutex_unlock(&m_pool_end);

        debug_info("[WORKERS] client: mpiServer_destroy_worker_pool(...) lock\n");
        pthread_mutex_lock(&m_pool);
        debug_info("[WORKERS] client: mpiServer_destroy_worker_pool(...) broadcast\n");
        pthread_cond_broadcast(&c_poll_no_empty);
        debug_info("[WORKERS] client: mpiServer_destroy_worker_pool(...) unlock\n");
        pthread_mutex_unlock(&m_pool);

        for (int i=0;i<MAX_THREADS;i++){
          debug_info("[WORKERS] client: mpiServer_destroy_worker_pool(...) join\n");
          pthread_join(thid[i],NULL);
        }

        debug_info("[WORKERS] client: mpiServer_destroy_worker_pool(...) destroy\n");
